# /doctor-ezactions — diagnose and self-heal the ezgha runner fleet

> **Use `doctor-runner`, not `doctor.sh` — the latter is broken on docker 27+ (see bead ez-gh-actions-91r + memory ezgha-doctor-idle-bug).** `doctor.sh` is kept only as a back-reference; the authoritative file is `doctor-runner` (shipped 2026-07-08, per-slot explicit-work inventory section 10; extended 2026-07-09 to the 4-state EXECUTING/IDLE-OK/IDLE-STARVED/DOWN activity-truth model — bead jleechan-hca0).
...
1. **Run the health check** (always includes queue metrics):
   ...
   **Section 8 — queue health** (from `scripts/queue-health.sh`):
   ...
   - Oldest fresh queued run (actionable backlog), incl. `QUEUE_OLDEST_FRESH_AGE_MIN` — how long the queue has actually been waiting (the sustained-duration signal section 9 uses for IDLE-STARVED)
   ...
   **Section 9 — per-slot activity truth** (LOCAL-ONLY, no GitHub API — the
   API lies under rate limit and can report a fully-busy fleet as "0
   listening"). Every configured runner on both hosts (local docker + the
   other host over SSH) is classified into exactly one of four states via
   `docker top <container> | grep Runner.Worker`:
   - **EXECUTING** — `Runner.Worker` process present. Never a defect.
   - **IDLE-OK** — listening, and either nothing is queued or the queue has
     been non-empty for less than `IDLE_STARVED_THRESHOLD_MIN` (default 5)
     minutes. Never a defect — a healthy idle fleet reads IDLE-OK, not down.
   - **IDLE-STARVED** — listening while the queue has been non-empty for
     `>= IDLE_STARVED_THRESHOLD_MIN` minutes (`QUEUE_OLDEST_FRESH_AGE_MIN`).
     Defect — contributes to `SLOT_PROOF_CRITICAL`.
   - **DOWN** — no running container. Always a defect.

   **Section 10 — EXPLICIT-WORK INVENTORY**: lists every configured slot on
   both hosts by name with evidence — EXECUTING slots show `job=<name>
   repo=<repo> elapsed=<Xm> url=<run URL>` (via the GitHub jobs API, matched
   on `runner_name` — the workflow-run object has no such field, so this is
   NOT the same lookup as the org runners list); IDLE slots show IDLE-OK
   ("nothing queued" or "queued but under threshold") vs IDLE-STARVED with
   the queue age. Also surfaces the serve-loop-starvation signal (max gap
   between respawn bursts, rate-limit occurrence count) — a gap over 150s
   means `ensure_count` is being starved by a rate-limited monitor tick (see
   `.claude/skills/ezgha-doctor/SKILL.md` Step 2b and bead
   ez-gh-actions-yrt/g3o).
