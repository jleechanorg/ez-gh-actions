...
>
> **UPDATE (2026-07-07 ~19:35 PT, job-level re-measurement done):** with proper JOB-level `started_at→completed_at` timing, 3 of the 4 disputed values turned out fine — the original run-level inflation was queue-wait contamination, as suspected. Measured: codex-skill-sync job p50=0.98min/max=2.80min (5min cap shipped in PR #8232 is SAFE); daily-gcp-cost-report max=7.20min and daily-gh-cost-report max=10.45min (15min caps SAFE). **Exception: daily-campaign-report.yml has GENUINE >15min executions** (job-level p50=3.23min but max=97.48min, 2/10 runs exceed 15min) — its 15min cap from #8232 would kill real runs; fix PR raising it to 120min is in flight, plus an investigation bead for the 97min outlier.
