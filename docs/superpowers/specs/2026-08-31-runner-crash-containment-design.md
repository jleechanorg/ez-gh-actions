...
Host Docker at `unix:///var/run/docker.sock` is an allowed production backend. It is not described as VM-contained. The Linux service does not require or auto-start `lima-vm@colima.service` when the host socket is selected. An inactive legacy Lima unit may be disabled after the backend endpoint is proven; no VM disk is deleted.
