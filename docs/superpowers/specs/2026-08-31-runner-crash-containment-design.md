...
Rust owns a short-lived opaque `AdmissionPermit` through one canonical containment implementation. `ezgha containment-check --install-static --json` exposes a read-only verdict for the persisted endpoint, hardware, privilege, fresh-versus-upgrade state, current use where applicable, and rollback storage without requiring a reachable VM daemon. For an explicitly configured recoverable Colima/Lima endpoint only, that verdict may authorize the separate exact-backend recovery decision. `ezgha containment-check --install-dynamic --json` then exposes a read-only verdict for reachable endpoint topology, Docker's cgroup driver, and `oomctl`; both install verdicts must pass before image work but neither returns an admission capability. `--admission` exposes the stricter read-only verdict after the effective installed cgroup/oomd policy and versioned helper receipt also pass. Internal Rust callers use that same implementation to obtain an in-memory `AdmissionPermit` with private fields and expiry; the CLI JSON never serializes or accepts a permit, so an environment variable, CLI argument, or shell-generated JSON cannot forge one.

Every Linux runner-admission-side mutation must obtain a fresh in-memory `AdmissionPermit` immediately before its first mutation: direct `start`, direct `serve` before pre-pull, every `ensure_count` reconciliation tick, and `install-service`. The host installer must first receive successful static and dynamic install verdicts before image acquisition or live mutation, then install and verify the policy/receipt, then receive the admission verdict before invoking `install-service`; `install-service` independently obtains and consumes the actual in-memory permit. A permit is never cached across ticks, retries, endpoint changes, or Docker mutations. The shell startup assertion remains defense in depth and consumes only the canonical admission verdict; it does not own a second decision table. Host Docker never invokes VM/Lima recovery, even when a check fails. A failed check returns a typed diagnostic and performs no admission-side Docker, service, config, or live-limit mutation; the sole pre-admission mutation exception is an exact configured Colima/Lima start authorized by a successful static verdict and separate `RecoveryDecision`. Stop/removal remains available for pressure relief.

Pre-daemon recovery is not admission. A separate Rust `RecoveryDecision`, derived only from the persisted normalized endpoint and platform, may start only the exact explicitly configured Colima/Lima backend through the existing bounded command. Canonical host, Docker Desktop, remote, inherited-context, indeterminate, and unsupported-platform endpoints produce `NoRecovery`; a stopped Docker Desktop daemon therefore returns a typed transient dynamic-preflight failure and is never auto-started. After a permitted VM start, full classification and an `AdmissionPermit` are still required before pre-pull or runner reconciliation. Durable pressure relief remains independent of admission: stop `ezgha.service`, then run `ezgha stop`; container removal alone is temporary while a supervisor can refill.

### Backend topology

Host Docker at `unix:///var/run/docker.sock` is an allowed production backend. It is not described as VM-contained. Add a backward-compatible defaulted `[backend].docker_endpoint` field to the deny-unknown config schema. New Linux install/admission requires a recognized explicit value and rejects missing, inherited-only, remote, or ambiguous endpoint state; `ezgha init` persists the normalized recognized endpoint it classified. Existing Mac configs without the field retain their current nonfatal compatibility behavior. One endpoint-aware `BackendTopology` classifier, owned by `src/backend.rs` and exposed as structured `ezgha topology --json`, combines that persisted endpoint with Docker daemon metadata: the canonical host socket plus a same-kernel Linux daemon is `HostDocker`; a recognized explicit Colima, Lima, or Docker Desktop socket plus VM daemon metadata is `VmDocker`; contradictory metadata, remote endpoints, and unknown sockets are `Indeterminate`. `Platform.daemon_in_vm` is only one metadata input and is never the classifier by itself. Shell gates consume the structured Rust result and do not duplicate the endpoint decision table. A Linux install/start gate rejects `Indeterminate`, while existing Mac and pre-existing runtime paths preserve their non-fatal warning/no-op behavior. The generated unit always emits the config endpoint, including `unix:///var/run/docker.sock`, and clears inherited `DOCKER_CONTEXT`; every Rust Docker subprocess uses that same value and removes `DOCKER_CONTEXT`. The generated Linux service has no unconditional `lima-vm@colima.service` dependency, and backend recovery must never start Lima for `HostDocker`; a failed HostDocker permit is a hard no-op, never a VM recovery request. Only explicit Colima/Lima endpoints may use the existing bounded recovery path; Docker Desktop is classifiable when reachable but is non-recoverable by this implementation when stopped. An inactive legacy Lima unit may be disabled after the backend endpoint is proven; no VM disk is deleted.
...
The workload-only oomd policy is intentional: Ubuntu's vendor `user@UID.service` kill behavior is the broad crash blast radius being removed, not a useful workload safeguard. All desktop ancestors are explicitly `auto`; `oomctl` must show only the finite workload roots (and an active proof root) as kill-capable roots. The proof must demonstrate that a workload victim does not terminate the user manager, Warp, or unrelated desktop processes.
...
`docs/verify-exit-criteria.sh` verifies effective values rather than file presence. Host-Docker and Colima gates are mutually exclusive: the host path checks the system `actions.slice`, the `systemd` Docker cgroup driver, actual cgroup ancestry for every managed container, user workload caps, the `user@` OOM policy and all desktop ancestors, effective `oomctl` roots, and the absence of unlimited/runtime overrides and `ezgha.service` OOM exemptions. It also validates the versioned helper receipt against installed paths, modes, and digests. It does not include an inactive Lima slice in host-Docker budget arithmetic or require a live guest. `doctor-runner` reports the same topology without mutating it.

## Data Flow

1. `install.sh` may build the exact local candidate binary locally (the build cannot invoke Docker), then requires a successful static install verdict. For an explicit recognized Colima/Lima endpoint on a supported platform only, it may start that exact VM, then requires the dynamic Docker-backed install verdict. Host Docker and Docker Desktop never recover a VM. Both phases must pass before image acquisition or any other Docker/live mutation, and neither verdict is an admission capability.
...
3. Only after containment and the helper receipt are effective may `ezgha containment-check --admission --json` return success; `ezgha install-service` then independently obtains and consumes an in-memory `AdmissionPermit` before regenerating the active user unit with the exact binary, config, startup gate, and normalized endpoint. Direct `serve` obtains another fresh permit before pre-pull, and each `ensure_count` tick obtains one before admission-side Docker reconciliation. The startup wrapper runs the canonical read-only admission verdict and then replaces itself with `ezgha serve`; any failure restores the prior unit, helper receipt, and active state together with the containment transaction.
...
- Cover host Docker, VM Docker, contradictory endpoint/daemon metadata, fresh/upgrade/mixed installation state, missing upgrade slice, infinite limit, wrong cgroup parent, inaccessible `oomctl`, vendor user OOM policy still active, desktop ancestors incorrectly enrolled for kill, unlimited agent override, an unscoped supported agent PID, any `AGENT_SLICE_OPT_OUT` path, a live `ezgha.service` OOM exemption, expired/forged admission permits, incomplete or mismatched helper receipts, and byte-exact rollback of both generated user units. Verify all six runner labels are emitted and that a registration-scope collision check proves the four-label capacity selector matches exactly the expected ten runner IDs/names.
...
7. Workload aggregates are enrolled for memory-pressure handling at 50%; `-.slice`, `user.slice`, the deploy owner's `user-UID.slice`, `user@UID.service`, and all desktop ancestors report both `ManagedOOMMemoryPressure=auto` and `ManagedOOMSwap=auto`, unprivileged `oomctl` lists only the finite workload roots (plus the active proof root), and `ezgha.service` has neither `ManagedOOMPreference=omit` nor `OOMScoreAdjust=-1000`.
8. The three explicit `ezgha containment-check` modes and the internal gate share one Rust implementation; bare `containment-check --json` is rejected rather than defaulting to a weaker mode. The installer requires successful read-only static and dynamic install verdicts before image/live mutation and an admission verdict before service installation/start; only the static verdict for an explicit recoverable Colima/Lima endpoint may authorize an exact backend start between those phases. Docker Desktop is classified when reachable but returns `NoRecovery` when stopped. Direct start, direct serve pre-pull, every `ensure_count` tick, and `install-service` independently require a fresh in-memory `AdmissionPermit`. The CLI never serializes a permit. Stop/removal stays available, the shell wrapper is defense in depth, and HostDocker never VM-recovers.
9. `ezgha.service` refuses startup on a proven host-Docker containment violation and can recover from a transient Docker query failure without exhausting its start limit. The versioned helper receipt is atomically installed, validates deployed SHA/source and installed paths/modes/digests, and rolls back together with its helpers.
...
12. No new watchdog, host restart authority, VM fallback, or runner-count reduction is introduced.
13. All controls, installation steps, verification logic, and the crash-containment tenet are git-tracked and reproducible on a fresh Ubuntu host.
