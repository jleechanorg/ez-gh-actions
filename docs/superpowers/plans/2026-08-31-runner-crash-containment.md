...
- Source work may land in green checkpoints, but production has exactly one atomic activation after Tasks 1-6 are complete. No interim may apply only oomd policy, workload limits, admission guards, image/config changes, or service ownership; Task 7 fences, drains, commits, verifies, and either starts the complete contained deployment or leaves it disabled/inactive `RecoveryRequired`.

## Activation Sequencing

This is intentionally one availability-focused cutover, not a narrow policy-only patch followed by a security release. Disabling Ubuntu's broad oomd targets before every workload cap and admission guard is effective can redirect pressure into global OOM. Installing limits without closing direct start/refill and competing mutators leaves bypasses; changing config, service ownership, and image identity without one fence makes rollback ambiguous. The broker, bundle authorization, descriptor handoff, effect gates, multi-platform immutable image, and receipt are therefore prerequisites for deterministic activation and recovery. They do not change runner scheduling or per-job lifecycle semantics. Tasks 1-6 may push inert source/artifact checkpoints, but no live systemd, Docker, config, or fleet mutation occurs before the sole Task 7 deployment.
...
- Produces: one public atomic installation entrypoint, a tracked pre-deployment administrator bootstrap for the fixed privileged broker/sudoers/bundle-authorization contract, shell-owned preflight/local-build/staging/dispatch ordering, explicit recovery-confirmation routing, retirement of the conflicting watcher/exemption stack and every independently scheduled mutating queue reaper, a closed declarative effect registry, read-only topology/reaper diagnostics, and a CI-enforced hard harness gate. Task 5 owns `install.sh` command grammar and its final exact `exec sudo -n` into Task 4's fixed privileged broker, including `--recover-transaction --confirm-transaction <nonce>` and receipt-independent/toolchain-independent `--clear-relief --confirm-request <id>`; it never owns quiescence, control locks, durable state, Rust handoff logic, or any live mutation primitive.
...
The Task 5 inventory contains 57 paths after adding the two bootstrap artifacts plus active `GEMINI.md` and `DESIGN.md`; the runner-image publisher and lock are already green in Task 4D/4E. Task 4 must be fully green first, and no live deployment or pressure proof runs between these checkpoints. The disjoint partition is 5A=10, 5B=37, 5C=6, and 5D=4.
...
git add install.sh scripts/host/install-crash-containment.sh scripts/host/bootstrap-ezgha-sudoers.sh \
  ...
  tests/install_crash_containment_test.sh tests/host_control_artifacts_test.sh tests/host_ops_0725_test.sh \
...
git diff --check
git add config/executable-effect-classes.toml tests/crash_containment_effect_inventory_test.sh \
...
- Produces: a timestamped evidence directory under `evidence/crash-containment/<timestamp>/` with separate `hard-limit`, `oomd-selection`, `ten-worker`, `attestation-calibration`, and `survival` verdicts.
...
This task is the only live activation. It requires all prior checkpoints, the hosted multi-platform image artifact, lock, broker authorization, and non-live suites green before quiescence. The service remains inactive throughout drain/cutover; no partial policy or legacy supervisor is activated. Failure restores only a complete receipt-proven prior deployment through the documented handoff or remains disabled/inactive `RecoveryRequired`.
...
# Set EVIDENCE_DIR to the exact directory printed by that successful command.
...
Expected: calibration mode dispatches exactly one captured 2700-second `capacity-proof.yml` run, records ten simultaneous managed containers whose direct `docker top` output includes `Runner.Worker`, correlates each container to the captured run/attempt through its exact marker and unique matrix number, proves actual cgroups are beneath `/actions.slice` at every one of 100 consecutive production attestation completions, and watches that run to success without cancellation. The verifier accepts only the current boot/receipt/config/run-bound schema-1 artifact, rank-99/lag/fast-guard thresholds, and current handoff timing records. The hard-limit proof records a local cgroup OOM; the oomd proof records both child pressure above threshold and a `systemd-oomd` victim beneath `ezghaproof-oomd.slice`; boot ID, user-manager PID, Warp, Docker, and unrelated runners survive both; the host-Docker Gate 8 passes without invoking Lima; and the monitor verdict is `HEALTHY`. No undefined duration-regression claim is made because the 2000% aggregate quota equals the ten configured 2-CPU maxima.
...
git add CLAUDE.md AGENTS.md README.md roadmap/up-changelog-20260831-crash-containment.md \
  ...
  install.sh .github/workflows/runner-image.yml config/runner-image.lock scripts/cleanup-stuck-runs.sh scripts/queue-backlog-drain.sh scripts/queue-health.sh config/README.md config/config.toml.linux.example config/config.toml.linux-canary.example config/config.toml.mac.example config/executable-effect-classes.toml doctor.sh doctor-runner \
