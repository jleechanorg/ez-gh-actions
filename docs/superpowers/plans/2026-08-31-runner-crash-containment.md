...
- **Pressure relief remains independent:** outside a deployment transaction, durable relief is `systemctl --user stop ezgha.service` followed by ordinary `ezgha stop`. While the transaction owns `serve.lock`, use only the tracked `ezgha emergency-stop --confirm` abort path; add `--cancel-running` only to remove locally proven managed containers. Relief never starts a backend/service, refills runners, or calls GitHub.
...
When invoked from the installed stable path, validate the versioned deployment receipt rather than comparing with a mutable checkout. The atomically written receipt records the deployed Git SHA and, for final `config.toml`, the `ezgha` binary, `assert-crash-containment.sh`, `ezgha-start-gate.sh`, root-owned `proof-allocator.sh`, generated `ezgha.service`, and generated `ezgha-alert@.service`, the artifact role, candidate/source path, installed absolute path, mode, and SHA-256. The config entry additionally records parsed `runner.count`, which must equal `10` on the fixed Linux profile, and requires mode `0600`. A missing/malformed receipt or any installed path, mode, digest, or parsed-count mismatch is a policy violation. Changing checkout files after installation does not invalidate an otherwise intact deployment; rollback restores or removes the receipt and every listed artifact, including config, together.
...
- Produces: canonical `ezgha topology --json`, canonical read-only `ezgha containment-check --install-static|--install-dynamic|--admission --json` verdicts, synchronous full `RuntimeAdmission`, background non-capability `RuntimeAttestation`, fresh tick-scoped `TickAdmission`, and private `TransactionAdmission` from one Rust decision implementation; a deployment-state classifier; a canonical lock primitive; `ezgha emergency-stop --confirm [--cancel-running]`; primitive `install-crash-containment.sh --preflight|--check|--install|--rollback RECEIPT`; and a generated endpoint-correct startup gate. Static install preflight proves the proposed endpoint, hardware, privilege, `Bootstrap`/`ManagedUpgrade`/`LegacyMigration`/`Mixed` state, current use where applicable, and rollback storage without requiring a reachable VM daemon. Only a recognized proposed Colima/Lima endpoint on a supported platform may then start that exact VM; Docker Desktop returns `NoRecovery`. Dynamic preflight proves the reachable daemon topology/driver and `oomctl` before image work. Neither verdict is an admission capability. Runtime admission requires the effective installed profile, receipt-backed count 10, and every final receipt artifact. The single-flight background attestor refreshes that proof outside foreground budgets; each runner-creation boundary consumes only a fresh cheap permit tied to an unexpired generation. Transaction admission additionally requires the inherited lock FD and candidate directory, validates final non-unit artifacts plus staged receipt-bound units, and can authorize only unit installation/reload. No scope is serialized through the CLI. Task 4 does not edit `install.sh` or retire legacy watcher/exemption state; Task 5 is the sole transaction and quiescence orchestrator and extends the primitive rollback receipt serially.
...
Promote the existing canonical `serve.lock` to the installation fence. `start`, `serve`, `stop`, and the transaction all use the active config's `state_dir`; the static-verdict JSON returns the normalized lock path, so shell does not parse TOML or reconstruct it. Release builds ignore/reject `EZGHA_SKIP_LOCK`, with test injection available only under `cfg(test)`. Managed and legacy upgrades reject proposed changes to `state_dir`, target/scope, or runner prefix before quiescence. Task 5 stops the old unit, requires inactive/MainPID exit within 30 seconds, opens a fixed inheritable descriptor, explicitly clears `FD_CLOEXEC`, acquires exclusive `flock` within 15 seconds, and retains that open-file description through success or rollback. The drain primitive and deferred service installer accept its number only through `--transaction-lock-fd <n>`. Rust bounds-checks the descriptor, matches `fstat` identity against a freshly opened canonical lock file, requires nonblocking exclusive `flock` on the inherited descriptor to succeed, then requires the same operation on a second canonical open-file description to fail with `EWOULDBLOCK`. The first call is idempotent for the inherited shared open-file description, atomically acquires an accidentally unlocked descriptor, and rejects a third-party owner; Rust never unlocks or closes it. Reject wrong-inode, closed, exec-lost, or noncontending descriptors. Legacy drain has a fixed 30-minute ceiling and never cancels a busy job. Add `install-service --defer-start --transaction-lock-fd <n> --unit-candidate-dir <path>`, valid only with the inherited FD, receipt-bound staged unit candidates, and a fresh transaction-scoped permit, to install both units without starting them. Fixtures pin `stop -> inactive -> lock -> first mutation`, exact timeouts, parent-held and atomically acquired FD inheritance, unlocked-with-third-party-holder rejection, wrong-inode/closed/exec-lost rejection, parent ownership after child exit, no nested-lock deadlock, busy-job preservation, lock-timeout zero-write behavior, deferred generation, one final start after release, and restoration/start only for a formerly active service.
...
Direct `start`, `serve`, and `install-service` perform one bounded full `RuntimeAdmission` synchronously before their first mutation; direct `serve` does so before pre-pull. A long-lived `serve` then owns one non-overlapping background full-attestation worker. A new read-only run starts no later than 20 seconds after the prior completion, has a 15-second wall-clock deadline including child processes, and checks Docker, systemd, `oomctl`, effective cgroups, receipt/config modes and SHA-256 digests, endpoint/topology, fixed count, and policy. It atomically publishes a non-capability snapshot with status/class, generation, completion time, boot ID, endpoint/count, and receipt/config metadata identity. A successful snapshot expires 45 seconds after completion; the foreground never joins the worker or charges it to the 30-second reconciliation cadence or shared 75-second monitor budget.

Every runner-creating tick mints a new in-memory `TickAdmission` from an unexpired successful snapshot plus a subprocess-free metadata/boot/config fast guard. The guard performs no Docker, GitHub, systemctl, `oomctl`, or SHA operation and has a live p99 budget of 100 ms. The permit expires at the earlier of 30 seconds after issuance or snapshot expiry, carries the snapshot generation, and is type-required by `ensure_count` and every reserve/JIT/container-create transition. Cleanup may proceed without it, but `assert_current()` must pass immediately before each runner admission; a newly published failure/generation revokes old permits.

Add shared atomic `AdmissionControl` plus cooperative cancellation to every serve-loop phase. Check it at the top of settle, reconcile, and monitor iterations, before/after every bounded external operation, and inside waits at least once per second. Serve-mode child processes accept the token; one-shot network/system calls have hard deadlines no greater than 15 seconds. Proven policy drift sends one critical typed alert, cancels foreground work, and exits `78` within 15 seconds. At snapshot TTL expiry, send a dedicated cooldown-deduplicated `containment.attestation.stale` alert on first observation, then also advance `ensure_fail_streak`/`notify_ensure_failure` as supplemental escalation. Stale state gives no dead-man credit, never restarts the backend, and exits `75` after two denied foreground observations or 75 seconds from `StaleSince`, whichever comes first; cancellation makes the wall-clock bound effective during long reconciliation. Tests inject clock/cancellable probes to pin cadence, deadline, TTL, revocation between two starts, no overlap, no foreground join, top-of-loop and per-operation observation, cancellation deadlines, first-stale alert independent of configured threshold, no forbidden fast-guard calls, latency, exit behavior, and healthy-pass streak reset.
...
Add `ezgha emergency-stop --confirm [--cancel-running]` as the only tracked pressure-relief path that bypasses `serve.lock`. Before waiting for `serve.lock`, the transaction acquires canonical `emergency.lock`, publishes a mode-`0600` `Preparing` record with schema/generation/nonce, installer PID/start time, boot ID, intended canonical lock path, config fingerprint, request path, phase, and prior service state, then releases the control lock. It never waits for the runner fence while holding `emergency.lock`. After acquiring `serve.lock`, it reacquires the control lock and either acknowledges a matching request or transitions the same nonce to `Fenced` with actual lock identity before release. This state machine linearizes initial publication, fence ownership, later phase transitions, request publication/acknowledgment, and final service handoff. The emergency command acquires the control lock, writes a mode-`0600` request, releases the lock, then awaits acknowledgment without retaining any lock; this permits the transaction to acknowledge instead of deadlocking. It may stop `ezgha.service` and never starts/restarts a backend or service, registers/refills/creates runners, changes config/policy/receipts, calls GitHub, releases slots, or deletes registrations. Without `--cancel-running` it requests/awaits transaction abort and removes no running container.
...
Before each bounded operation, the installer holds `emergency.lock`, checks the request, and records the next phase. Abort linearizes when it acknowledges a matching request by atomically setting `aborting`; no runner-capable operation begins after that point. It checks after each operation and once per second during drain. The final handoff holds `emergency.lock` across the last check, `systemctl --user start`, and terminal commit. If the transaction wins that lock, the waiting emergency command subsequently stops the new service; if emergency wins, start is suppressed. Abort rolls back under `serve.lock`, deliberately leaves the old supervisor inactive, records `aborted`, and releases.

Emergency waits at most 35 seconds for control-lock/acknowledgment and reports incomplete relief on timeout after a bounded service-stop attempt. If no transaction state is visible while holding `emergency.lock`, it probes the canonical `serve.lock` through a separate nonblocking exclusive `flock`. Success is immediately unlocked/closed and permits a standalone request to complete/remove; `EWOULDBLOCK` becomes `OrphanedFence`, persists the request, stops the service, and returns incomplete rather than losing intent. Terminal, stale-boot, dead-PID, malformed, or interrupted visible state permits bounded stop/optional proven-local removal but does not clear or declare the transaction successful. Ordinary install/start stays fail-closed until tracked `./install.sh --recover-transaction` uses the same control-lock/nonblocking-fence protocol without waiting for the fence under the control lock. Valid stale transaction state plus a saved snapshot takes the verified rollback branch. An orphan request with no state/snapshot takes a separate branch requiring inactive `ezgha.service` and successful nonblocking fence acquisition; while holding that probe it records `orphan-cleared`, removes only the validated orphan request, performs no rollback/success claim, releases the probe, and never starts a service. A busy fence or active service remains fail-closed. Ordinary `ezgha stop` remains lock-serialized outside a transaction. Fixtures cover request publication releasing `emergency.lock` before acknowledgment wait, transaction acknowledgment progress, emergency before `Preparing`, during `Preparing`, the exact gap before/after runner-fence acquisition, hidden/orphan fence persistence, busy/active orphan recovery rejection, holder-exited orphan clearing without snapshot or start, request interleavings at every later phase and within each bounded operation/drain, the exact final-start race, abort acknowledgment, formerly-active service left stopped, stale/malformed/interrupted/no-active lifecycle and timeout, Linux/Darwin service stubs, label/runner-ID/slot ownership, ambiguous/sibling skips, default no deletion, partial removal, zero GitHub calls, and zero backend start/refill/create calls while allowing only approved inspect/removal.
...
After both verdicts, the transaction snapshots the installed binary bytes/path/mode/digest; live config; every old policy/helper file; the deployment receipt; exact generated `ezgha.service` and `ezgha-alert@.service` units; the live `10-oomd-omit.conf`; the installed legacy watcher service unit, timer unit, and stable script; and the applicable active/enabled states of `ezgha.service`, `ezgha-alert@.service`, `psi-oom-watcher.service`, and `psi-oom-watcher.timer`. Before image acquisition or binary/config/helper/receipt/policy/manager mutation, it performs upgrade quiescence: verify the proposed upgrade preserves `state_dir`, target/scope, and runner prefix; stop the old `ezgha.service` only when active; verify inactive and old MainPID exit; under `emergency.lock` publish `Preparing` and release it; then use the static verdict's canonical path to open a fixed inheritable `serve.lock` descriptor with a bounded timeout, explicitly clear `FD_CLOEXEC`, retain that open-file description, and under `emergency.lock` transition to `Fenced` or acknowledge a pending request. It never waits for `serve.lock` while holding the control lock. Managed-upgrade containers may remain only after proving their actual cgroups are beneath `actions.slice`. Legacy migration invokes the drain primitive with `--transaction-lock-fd <n>` for a bounded zero-container drain, never cancels a busy job, and removes only idle managed containers/registrations after busy work finishes. Rust validates numeric bounds and canonical inode identity, requires nonblocking exclusive `flock` on the inherited descriptor to succeed, then requires a second canonical descriptor to fail with `EWOULDBLOCK`; it never unlocks or closes the inherited FD. Drain/lock/FD-validation failure atomically records failure under `emergency.lock`; it restores the formerly active old service only when no emergency request exists and exits before image or installed/containment mutation. Fixtures prove the initial-publication/fence interleavings, parent-held and atomically acquired inheritance, parent ownership after child exit, and rejection of unlocked-with-third-party-holder, wrong-inode, closed, or exec-lost descriptors; no refill/JIT/GitHub/Docker admission call can occur after lock acquisition and before the new service starts; no nested-lock deadlock exists; and release `EZGHA_SKIP_LOCK=1` cannot bypass the fence.

Only after quiescence, the pre-fence `Preparing` publication, fence acquisition, and nonce-matched `Fenced` transition may the transaction acquire/build the image; stage and atomically install the candidate binary, mode-`0600` proposed config, and stable assertion/gate helpers; render both generated units without installing them; verify every installed or staged path/mode/SHA-256 plus parsed config count; atomically commit a versioned deployment receipt containing final config path/mode/digest/count, every non-unit artifact, and final unit paths/staged candidate digests; invoke the privileged installer including atomic watcher/exemption retirement; and verify effective containment and `oomctl` roots. Config and receipt-listed artifacts are one rollback unit: bootstrap removes them and upgrade restores their prior bytes. Before each bounded operation it holds `emergency.lock` long enough to poll the request and publish the next phase; acknowledgment atomically changes the phase to `aborting`, after which no runner-capable operation may begin. It checks after each operation and once per second during drain. Abort rolls back under `serve.lock`, leaves the old supervisor inactive, records `aborted`, and releases. Stale/malformed/mismatched state is fail-closed for start and recoverable only through `./install.sh --recover-transaction` as specified above.

Ordinary runtime admission must fail at the transitional point because final units are absent. The transaction then calls endpoint-preserving `ezgha install-service --defer-start --transaction-lock-fd <n> --unit-candidate-dir <path>`. That Rust command validates the inherited FD and receipt-bound staged units, obtains and consumes private `TransactionAdmission`, atomically installs and verifies both units, reloads the user manager, and has no type-level path to start, pre-pull, reconcile, register, or create runners. A subsequent ordinary `ezgha containment-check --admission --json` must validate every final receipt path before the transaction releases `serve.lock` for the single final `systemctl --user start` handoff. The transaction holds `emergency.lock` across the last request check, start, and terminal-state commit; if emergency obtains the lock first it suppresses start, while if transaction obtains it first the waiting emergency command stops the newly started service. It starts only when the selected prior/final state requires it and post-start verifies the unit, new MainPID, lock ownership, `/proc/<pid>/oom_score_adj`, and all receipt-bound digests. Any post-quiescence failure restores every recorded file byte-for-byte, including the old binary, helpers, receipt, and both generated units, reloads the relevant system and user managers, restores each unit's prior enabled/disabled state while retaining the lock, then releases it immediately before starting only units that were formerly active, and verifies rollback. Fixtures cover bootstrap, managed upgrade, legacy migration, mixed state, binary/helper/receipt/unit tampering, checkout changes after install, companion alert-unit rollback, stopped-Docker-Desktop `NoRecovery`, quiescence timeout, successful inherited-FD handoff plus every rejection branch, busy-job preservation, transaction-mode rejection of missing/wrong candidate path/mode/digest/FD, type-level runner-call rejection, deferred service generation, staged-to-installed digest equality, pre-install runtime-admission failure, post-install runtime-admission success, emergency requests at every phase and during every bounded operation/drain, exact final-start interleavings, stale/malformed/no-active/interrupted state recovery, default no-container-deletion, explicit partial proven-local removal, zero GitHub and backend start/refill/create calls while allowing only approved Docker inspect/removal, interrupted receipt-before-unit state returning `Mixed` with no start and exact rollback, single-start handoff, and exact rollback. An ordered fixture proves the restored old binary and `ExecStart`, not a failed generated command cached before reload, are launched. A failed rollback is reported distinctly and never as installation success. Do not add an unconditional Lima dependency or a second service-start path; an unrelated obsolete binary is explicitly outside the supported rolling-upgrade surface.
