...
Checkpoint 4D owns `.github/workflows/runner-image.yml` and `tests/runner_image_publisher_test.sh`. The test structurally validates the exact three push paths, GitHub-hosted-only runner, fixed public GHCR target, minimal package publication permissions, provenance-artifact schema, and prohibited self-hosted/socket/host-manager/VM/cgroup/runner-control edges. Run `bash tests/runner_image_publisher_test.sh` and `git diff --check`, then run `git add .github/workflows/runner-image.yml tests/runner_image_publisher_test.sh`, commit `codex/gpt-5.6-sol: publish immutable runner image`, and push. That workflow-path push triggers the hosted publisher and makes its digest artifact available without using the production runner fleet.

Checkpoint 4E owns startup/profile/image-lock integration: `scripts/host/ezgha-start-gate.sh`, `src/service.rs`, `src/containment.rs`, `src/config.rs`, `src/main.rs`, `src/alert.rs`, `config/config.toml.linux.example`, `config/runner-image.lock`, and `tests/runner_image_lock_test.sh`. Download the successful 4D run's machine-readable provenance artifact with `gh run download`, generate the lock structurally, require its publisher source commit to equal the prior 4D commit, and separately require its workflow/Dockerfile/context digests to match the current tree before adding it; the later lock/profile commit does not pretend to equal the publisher commit. The production Rust lock parser owns the same schema checks, and the shell fixture structurally requires the immutable repo digest, source/provenance fields, recomputed three current-tree digests, and Linux example image equality without Docker, network, or GitHub calls. Run shell syntax, `bash tests/runner_image_lock_test.sh`, generated-unit/service/alert tests, config/JIT exact-four-label tests, Rust lock-parser tests, `cargo test`, `bash tests/assert_crash_containment_test.sh`, and `git diff --check`; then run `git add scripts/host/ezgha-start-gate.sh src/service.rs src/containment.rs src/config.rs src/main.rs src/alert.rs config/config.toml.linux.example config/runner-image.lock tests/runner_image_lock_test.sh`, commit `codex/gpt-5.6-sol: wire contained service startup`, and push.
...
Implement `scripts/host/install-crash-containment.sh` here with exactly `--preflight`, `--check`, and `--stage`; remove or fail closed on legacy `--install`. `--stage` writes only to the fixed private bundle inbox and emits the complete canonical manifest consumed by the bootstrap/broker. Add a static shell scan plus execution traces proving the stager contains no `sudo`, and `install.sh` contains only the three exact final `exec sudo -n -- /usr/local/libexec/ezgha/ezgha-privileged ...` forms. Reject `sudo -E`, interactive sudo, a shell command string, any later sudo, a mutable sudo target, mutating `systemctl`, Docker image/runtime mutation, final-path write/remove, or rollback command in shell. Normal install dispatches `transaction`; recovery dispatches `recover-transaction --confirm-transaction <nonce>`; clearance dispatches `clear-relief --confirm-request <id>` without Cargo, a checkout lookup, config, receipt, or bundle staging. Missing noninteractive privilege fails before quiescence, locks, or live mutation.
...
The Task 5 inventory contains 55 paths after adding the two bootstrap artifacts; the runner-image publisher and lock are already green in Task 4D/4E. Task 4 must be fully green first, and no live deployment or pressure proof runs between these checkpoints. The disjoint partition is 5A=8, 5B=37, 5C=6, and 5D=4.
...
rg -n 'Crash-containment contract|Backend choice is subordinate|reset-failed ezgha.service|staged bootstrap state' CLAUDE.md AGENTS.md README.md config/README.md
git diff --check
git add CLAUDE.md AGENTS.md README.md config/README.md \
...
git add CLAUDE.md AGENTS.md README.md roadmap/up-changelog-20260831-crash-containment.md \
  ...
  install.sh .github/workflows/runner-image.yml config/runner-image.lock scripts/cleanup-stuck-runs.sh scripts/queue-backlog-drain.sh scripts/queue-health.sh config/README.md config/config.toml.linux.example config/executable-effect-classes.toml doctor.sh doctor-runner \
