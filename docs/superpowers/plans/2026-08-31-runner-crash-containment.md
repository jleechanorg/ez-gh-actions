...
- Docker-driver remediation is a separate tracked host-maintenance prerequisite, not part of ordinary installation: it requires an inactive service, zero managed containers, no transaction, and explicit acceptance that restarting Docker may interrupt unrelated containers.
...
- Source work may land in green checkpoints, but Linux production has exactly one atomic activation after Tasks 1-6 are complete. No interim may apply only oomd policy, workload limits, admission guards, image/config changes, or service ownership; Task 7 fences, drains, commits, verifies, and either starts the complete contained deployment or leaves it disabled/inactive `RecoveryRequired`. Task 7 has zero Mac-side effects. Task 8 is a separately owned atomic Darwin activation after Linux integration is green.

## Activation Sequencing

This is intentionally one availability-focused Linux cutover, not a narrow policy-only patch followed by a security release. Disabling Ubuntu's broad oomd targets before every workload cap and admission guard is effective can redirect pressure into global OOM. Installing limits without closing direct start/refill and competing mutators leaves bypasses; changing config, service ownership, and image identity without one fence makes rollback ambiguous. The broker, bundle authorization, descriptor handoff, effect gates, multi-platform immutable image, and receipt are therefore prerequisites for deterministic activation and recovery. They do not change runner scheduling or per-job lifecycle semantics. Tasks 1-6 may push inert source/artifact checkpoints, but no live Linux systemd, Docker, config, or fleet mutation occurs before the sole Task 7 Linux deployment, except the separately scheduled empty-fleet Docker-driver maintenance prerequisite. That prerequisite must complete and be re-preflighted before Task 7 and cannot apply any ezgha containment policy, runner mutation, bundle, or installed service. Task 7 cannot invoke launchd, a Darwin candidate, or Mac Docker/Colima. Task 8 later owns the independent six-runner Darwin receipt transaction.
...
- `scripts/host/bootstrap-docker-systemd-cgroup-driver.sh`: explicit empty-fleet host prerequisite that structurally establishes Docker's systemd cgroup driver.
...
- `scripts/job_outcome_monitor.py`: explicit timestamp-bounded pre/post release-health evidence with complete-population enforcement.
...
Create isolated branches/worktrees `codex/crash-policy`, `codex/crash-systemd`, and `codex/crash-assertion` before dispatch. Each lane commits and pushes only its branch; no lane pushes the integration branch. Task 4 begins after B and C because its installer and generated service consume both outputs. Task 5 begins after Task 4 and is the single writer for `install.sh`, watcher retirement, Docker-driver bootstrap, job-outcome monitor, shared operational tests, and CI wiring. Task 6 begins after Tasks 2-5 because its live proofs consume the final hierarchy; Task 5's CI suite discovers Task 6's fixture without another shared-workflow edit. Task 7 is the sole Linux live cutover, is strictly serial, and belongs to the Linux deploy owner. Task 8 is a later strictly serial Mac cutover owned by the Mac deploy owner; it cannot overlap Task 7.
...
- Modify: `config/config.toml.linux-canary.example`
- Modify: `config/config.toml.mac.example`
- Create: `.github/workflows/runner-image.yml`
- Create: `config/runner-image.lock`
- Create: `tests/runner_image_publisher_test.sh`
- Create: `tests/runner_image_lock_test.sh`
- Create: `tests/fixtures/topology/golden.jsonl`

**Interfaces:**
- Consumes: Task 2 unit files and Task 3 assertion.
- Produces: canonical topology and three read-only containment verdicts; synchronous `RuntimeAdmission`; private Darwin-only one-use `CandidatePromotionAdmission`, `MacImageBootstrapAdmission`, and `MacDeploymentCommitAdmission`; private non-serializable `MacDeploymentRecovery`; background non-capability `RuntimeAttestation`; `AdmissionIssuer` plus the sole `EffectCoordinator`; one-use `AdmissionMutation`; narrow `ReliefCleanup`; non-capability `AuditStateWrite`; private `TransactionAdmission`; deployment classifier; canonical runner/control locks; durable effect/relief/recovery state; receipt-bound descriptor handoff; primitive containment install/rollback; an effect-denying read-only production planner that emits `LegacyMigrationRehearsalV1`; and the endpoint-correct private systemd startup gate. Static/dynamic verdicts remain non-capabilities; only one exact explicit Colima/Lima start may recover. Runtime proof binds final artifacts/count, background refresh stays outside foreground budgets, and the coordinator exposes noninterchangeable `AdmissionGate`, `DeploymentGate`, `RecoveryGate`, and `ReliefGate` scopes that linearize every forward commit/spawn against the durable latch. The Darwin bootstrap scopes authorize only fixed retained-byte promotion, one exact lock-bound pull, one receipt-bound config/plist/receipt commit derived from its in-memory result, or verified durable-record recovery to inactive state; transaction admission can only install/reload staged units, and the handoff gate transfers the continuously locked FD only to the proven systemd peer. No capability is CLI-serialized. Task 4 owns the private Rust install transaction, quiescence/fence/state ordering, `EffectCoordinator`, every gate/state type, the privileged deployment/rollback executor, generated-unit code, Rust recovery primitives, the complete private Rust `handoff-service` implementation including socket/FD transfer, deadlines, terminal-owner resolution, and failure transitions, the read-only rehearsal planner, and every admission, relief, or audit integration in the Rust modules listed above. Task 4 does not edit `install.sh`, expose recovery as a public shell command, or retire legacy state; Task 5 owns public shell preflight/build/stage/dispatch grammar and removal of external mutating reapers.

Task 4 implements and tests the design's normative monotonic timing table: full attestation `15s`; next start within `20s`; snapshot TTL `45s`; one-use permit `min(issue+5s, snapshot expiry)`; stale escalation at the earlier of a second denied foreground observation or `StaleSince+75s`; handoff connect `25s`; handoff ready `120s`; service `TimeoutStartSec=210`; race drain `1800s`; and fleet convergence `600s` from `READY=1`. Rust fake-clock tests pin `20+15<=45`, the 10-second refresh margin, permit expiry at the snapshot boundary, stale escalation independent of the queue monitor's 75-second budget, the 145-second absolute handoff boundary with 65 seconds of systemd margin, ten-versus-fewer replacement containers at the 600-second convergence boundary, and the cumulative `1800+210+600=2610s` successful-cutover limit from first legacy removal. Release acceptance additionally requires the design's full-load sample: at least 100 consecutive production-scheduled attestations with rank-99 duration at or below 5 seconds, maximum scheduling lag at or below 20 seconds, fast-guard rank-99 at or below 100 ms, and the current receipt-bound assertion/provenance and pre-pull/READY records at or below 10/60 seconds. Any miss blocks release acceptance and invokes tracked rollback; the exact enforcement remains active throughout. Attestation/staleness denies new mutation and may restart the supervisor, but never cancels or removes an already-running job.

Add a bounded `AuditStateWrite` ring of at least 128 completed production observations and the read-only command `ezgha containment-calibrate --observe-only --samples 100 --capacity-run <captured-run-evidence.json> --output <path>`. Each ring record binds sequence, monotonic start/completion/lag/duration, outcome, boot ID, generation, and receipt/config identities. The command composes a private non-capability `ReadOnlyCapacityObserver`, limited to exact managed-container inventory, `docker top`, captured marker read, and `/proc/<pid>/cgroup`, with a separate `AuditStateWrite` sink limited to atomic ring/artifact persistence. Neither API can construct, hold, or call the other's interface. The command consumes the existing scheduler's consecutive observations; it never launches a second loop, publishes an attestation, mints a permit, changes a revocation epoch, alerts/cancels/restarts, takes an effect gate, or invokes a Docker lifecycle, GitHub, or manager mutation. At every consumed sequence the observer keys its result to the attestation sequence and requires the same ten captured `Runner.Worker` containers beneath `/actions.slice`; only the sink writes schema-1 `attestation-calibration.json`. The artifact contains all raw samples, sequence-keyed capacity observations, exact thresholds, capacity run ID/attempt/workflow SHA, boot/artifact identities, receipt-bound handoff timing records, and summary. Define p99 as sorted rank `ceil(0.99*n)`. Tests cover the 100th-sample boundary, rank-99 5000/5001 ms, lag 20000/20001 ms, timeout/failure/gap/restart/identity mismatch, insufficient or mismatched capacity evidence, missing handoff record, atomic-output failure, and spies proving zero permit/gate/Docker-lifecycle/GitHub/manager/alert/revocation effect while permitting only those exact read-only Docker/process observations. Compile-time/API fixtures prove `AuditStateWrite` cannot invoke the observer or Docker and the observer cannot write state or obtain any supplied effect interface.

Add an argument-free fixed-broker `rehearse-transaction` grammar. It reads the already authorized descriptors and the same live-state inputs as `transaction`, runs the exact production parser/classifier/planner plus rollback-space and timing calculations with an effect-denying coordinator, and emits canonical `LegacyMigrationRehearsalV1` JSON to stdout. The schema binds boot ID, observed config digest, bundle authorization digest, broker digest/protocol, deployment class, prior service state, exactly ten expected managed container names/IDs, `managed_container_count=10`, `worker_count=0`, idle-proof timestamp, planned effect sequence, expected-prior fingerprints, rollback bytes/free space, drain/readiness/convergence bounds, and an expiry of five minutes. It cannot acquire `serve.lock`/`emergency.lock`, persist state, invoke any gate, stop/reload/start a manager, write a final path, pull/build/tag/run/remove an image/container, or call GitHub. The deploy workflow accepts only a rehearsal with exact current boot/config/bundle/broker identities and state fingerprints; any drift requires a new rehearsal. The live broker independently recomputes and rejects container/worker/state drift before its first removal. Tests use the production planner behind both modes, inject a panic if the rehearsal coordinator sees an effect request, compare the rehearsal sequence to a transaction fixture, and prove stale/substituted output cannot authorize anything.
...
cargo test containment::tests::legacy_migration_rehearsal
cargo test containment::tests::darwin_deployment_transaction
cargo test service::tests::darwin_receipt_startup
```

Expected: FAIL because the Rust deployment executor, effect-free rehearsal, and Darwin transaction/startup contracts do not exist.
...
Implement the fixed privileged broker at `/usr/local/libexec/ezgha/ezgha-privileged` as a root-owned non-symlink mode-`0755` installation of the tracked Rust binary. It reads only root-owned non-symlink `DeploymentBundleAuthorizationV1` metadata containing `schema`, broker protocol/digest, deploy UID/GID, passwd-derived containment root, fixed inbox identity, canonical `bundle_digest`, sorted `entries`, and canonical `runner_image`. Each entry binds role, `install|replace|remove`, exact final target, staged relative name, SHA-256 or required absence, mode, owner/group, and expected-prior fingerprint plus manager state. Entries cover candidate, config, receipt inputs, every helper/script/policy/drop-in/unit/generated-unit byte, every retired/remove target, and `config/runner-image.lock`. `runner_image` binds the lock's immutable GHCR digest, source commit, Dockerfile/context digests, and publisher provenance. It accepts exactly `transaction`, argument-free `rehearse-transaction`, `recover-transaction --confirm-transaction <nonce>`, and `clear-relief --confirm-request <id>` and rejects every path, shell command, environment override, unknown flag, and extra argument.

For `transaction`, the broker opens the authorization, manifest, and every staged entry without symlink traversal. It requires canonical manifest equality with the root authorization and no missing, extra, duplicate, target, operation, metadata, expected-prior, or `runner_image` difference; verifies the complete bundle digest; retains every verified descriptor; and commits only those retained bytes without reopening an artifact pathname. An authorized candidate paired with a substituted config/unit/policy/helper/image lock/digest/provenance/final target, `replace` changed to `remove`, changed mode/owner/expected-prior state, or rewritten/extra/missing manifest entry must produce zero effects. A pathname replacement after verification does not alter committed bytes. After fencing, the broker downshifts one exact digest-qualified `docker pull` child to the deploy UID through `DeploymentGate`, verifies the requested repo digest and local image ID, records both in the receipt, and runtime uses that ID. It rejects mutable tags and never invokes Docker build/tag or passes a checkout path. The broker executes the closed transaction state machine itself and installs the candidate only for later deploy-UID service execution; it never root-executes candidate or installed application code. Recovery and clearance also execute inside the broker. Normal transactions never replace the broker or authorization; any bundle input/effect or helper protocol change requires restaging and the tracked interactive bootstrap before quiescence. `install.sh` proves this exact `sudo -n` contract before quiescence and ends with `exec sudo -n -- /usr/local/libexec/ezgha/ezgha-privileged transaction`; it never sudo-executes a mutable checkout path. The broker validates the explicit deploy UID/GID and minimal receipt-bound environment, retains root only for system-manager and system-path effects, and spawns every user-manager or Docker operation after `setresgid`/`setresuid` with no arbitrary environment inheritance. Missing privilege, helper, bundle authorization, descriptor identity, or any mismatch exits with zero effects. Only then may it revalidate typed verdicts, snapshot replaced policies/helpers/generated units plus active/enabled intent, acquire and retain the canonical transaction-lock FD, and become the sole owner of live mutation. No gate accepts an arbitrary command, path, callback, environment, or shell-produced capability. Each final artifact rename/mode/owner change, policy/unit install/removal, exact image-digest pull, manager reload/enable, and allowed service-start spawn is one separately prepared `DeploymentGate` effect. Each rollback/stop/restore/reload is a separate latch-bound `ReliefGate` effect, starts nothing, and leaves durable `RecoveryRequired`/`Mixed` state rather than shell-retrying an ambiguous result. Neither shell surface calls `systemctl set-property`, and compatible prior active state may return only through receipt-proven descriptor handoff; pre-baton rollback remains disabled/inactive `RecoveryRequired`.
...
For legacy Mac configs only, add a bounded Darwin compatibility resolver. In order, it considers a nonempty explicit override, the installed launchd plist endpoint, the active Docker context, and recognized Colima/Lima/Docker Desktop sockets; it accepts one normalized consistent endpoint and returns `Indeterminate` on conflict, unknown transport, or no evidence. Direct legacy runtime uses that in-memory result nonfatally until `ezgha init` or private `install-service --darwin-candidate`; `init` may write its starter config, while the Darwin candidate persists the endpoint only inside `MacDeploymentCommitAdmission`'s receipt-bound config/plist/receipt transaction. No independent install-service config write exists. After persistence the config wins over ambient state. Only a uniquely proven Colima/Lima endpoint may recover. Reachable Docker Desktop classifies as `VmDocker`, stopped Docker Desktop returns `NoRecovery`, and ambiguity starts nothing. Linux never invokes this resolver. Tests cover every precedence/conflict branch, prohibit a pre-transaction config write, and prove legacy Mac Colima recovery does not regress.
...
Except for private Darwin `install-service --darwin-candidate`, whose only pre-runtime mutations are retained-byte promotion, one exact lock-bound pull, and one atomic receipt-bound config/plist/receipt deployment commit under the three noninterchangeable bootstrap admissions after static/dynamic validation, direct `start`, `serve`, `install-service`, and `canary-once` perform one bounded full `RuntimeAdmission` synchronously before their first mutation; direct `serve` does so before pre-pull. Darwin obtains ordinary `RuntimeAdmission` immediately after the complete deployment commit and before launchd activation or runner work. A long-lived `serve` then owns one non-overlapping background full-attestation worker. A new read-only run starts no later than 20 seconds after prior completion, has its own 15-second deadline, and checks Docker, systemd, `oomctl`, effective cgroups, receipt/config modes and SHA-256 digests, endpoint/topology, fixed count, and policy. It publishes a non-capability snapshot with observation generation, revocation epoch, status/class, completion time, boot ID, endpoint/count, and receipt/config metadata identity. A successful snapshot expires 45 seconds after completion; the foreground never joins the worker or charges it to reconciliation or the shared 75-second monitor budget.
...
- [ ] **Step 5: Migrate profiles and image provenance**

In `config/config.toml.linux.example`, retain `count = 10`, set `[backend].docker_endpoint = "unix:///var/run/docker.sock"`, set `runner.image` to the exact digest-qualified OCI index reference from `config/runner-image.lock`, and define ordered `C = [self-hosted, self-hosted-mikey, ezgha, ez-runner-c]`. Set `runner_floor_mb = 2500`, set `limits.memory_mb = 2500`, and remove the explicit Colima-only `vm_total_mb`, `guest_reserve_mb`, and `host_reserve_mb` claims. Update `config/config.toml.linux-canary.example` to the same index reference and reviewed selector contract rather than leaving a mutable `latest`/`Linux`/`X64` exception. In `config/config.toml.mac.example`, retain the six-runner count, existing Mac label vector, Colima policy, and resource values, but set `runner.image` to that same index reference; do not hard-code a user-specific socket, and require the `linux/arm64` lock child. Config, every REST `generate-jitconfig` request, and every canary/capacity selector must equal `C` exactly for the fixed Linux fleet. Add typed `RemoteRunnerLabel { name, kind }` parsing to JIT and runner-list response models and retain all registry labels losslessly. Apply strict validation only to the newly created expected JIT runner and registry entries matching `C` as a subset: their custom labels are exactly `C - {self-hosted}`, and every remaining label is a recorded `type: read-only` supplement. Unrelated nonmatching Mac or sibling runners remain inventory evidence and do not fail list/reconciliation. Missing/unknown types, duplicate/empty names, missing `C` on an expected runner, or an additional custom label on a `C` match fail closed. Supplemental `Linux`/`X64` labels never flow into config, JIT requests, `runs_on_json`, or selectors. Update config/JIT argv/response fixtures and the existing `src/config.rs` example assertions, replace the obsolete dispatch test that selects `Linux`/`X64`, and add cases for supplemental read-only labels, reordered remote objects, missing `C`, duplicate labels, custom-label drift, and an unrelated custom-labeled runner that must remain valid inventory. The registry collision predicate is `C` as a subset, and it must still match exactly `ez-runner-c-1..10` and fail on any eleventh match. State the blast radius explicitly: this aligns the tracked fresh-machine default from 3000 to the already-live Jeff-Ubuntu value of 2500 MiB, a 16.7% reduction for new hosts that may expose memory-heavy jobs, preserves the existing label demand surface, and persists the already-effective host socket. Explain that the sum of tracked workload hard caps leaves about 8.5 GiB of budgeted headroom but does not cap desktop, kernel, Docker, or other system memory. Activation records the existing live value and must stop for reviewed profile sizing rather than silently lowering a host above 2500 MiB; release evidence requires the bounded job-outcome sample and ten-worker proof.

Create a GitHub-hosted `runner-image.yml` publisher that is the only supported build path for `Dockerfile.runner` plus its declared context. Its exact push-path trigger set is `.github/workflows/runner-image.yml`, `Dockerfile.runner`, and `docker/tar-workspace-wrapper.sh`; the workflow file is itself a provenance input, so the initial workflow-only 4D commit triggers publication. It uses Buildx to publish one public OCI index for exactly `linux/amd64,linux/arm64` at `ghcr.io/jleechanorg/ezgha-runner:<source-sha>`. Its machine-readable artifact records the immutable index digest and, for each exact platform, child manifest and image-config digests, plus publisher source commit and workflow/Dockerfile/context digests. A reviewed agent refreshes tracked `config/runner-image.lock` from that complete artifact; CI rejects a lock whose platform set or three input digests do not match the current tree. The hosted workflow is the sole build/push authority; every Linux or Mac host consumes only that lock and never builds or tags an image.

- [ ] **Step 6: Land green Task 4 checkpoints**

Task 4 remains serial because its shared Rust types are coupled, but it lands as five independently green commits. Do not combine these checkpoints or run writers concurrently.
...
Checkpoint 4D owns `.github/workflows/runner-image.yml` and `tests/runner_image_publisher_test.sh`. The test structurally validates the exact three push paths, GitHub-hosted-only runner, fixed public GHCR target, exact Buildx platform set `linux/amd64,linux/arm64`, minimal package publication permissions, index/child/config provenance schema, and prohibited self-hosted/socket/host-manager/VM/cgroup/runner-control edges. Run `bash tests/runner_image_publisher_test.sh` and `git diff --check`, then run `git add .github/workflows/runner-image.yml tests/runner_image_publisher_test.sh`, commit `codex/gpt-5.6-sol: publish immutable runner image`, and push. That workflow-path push triggers the hosted publisher and makes its digest artifact available without using the production runner fleet.

Checkpoint 4E owns startup/profile/image-lock integration: `scripts/host/ezgha-start-gate.sh`, `src/service.rs`, `src/containment.rs`, `src/config.rs`, `src/main.rs`, `src/alert.rs`, all three config examples, `config/runner-image.lock`, and `tests/runner_image_lock_test.sh`. Download the successful 4D run's machine-readable provenance artifact with `gh run download`, generate the lock structurally, require its publisher source commit to equal the prior 4D commit, and separately require its workflow/Dockerfile/context digests to match the current tree before adding it; the later lock/profile commit does not pretend to equal the publisher commit. The production Rust lock parser owns the same schema checks, and the shell fixture structurally requires the immutable index digest, exact two-child platform set and child/config digests, source/provenance fields, recomputed three current-tree digests, and equality of all three example images without Docker, network, or GitHub calls. Rust tests select amd64/arm64 by daemon platform and reject missing/extra/wrong-platform children, architecture mismatch, local-ID substitution, and every failure before runner/container mutation. On Darwin, shell stages only the candidate and manifest in fixed `<containment-root>/darwin-install-inbox`, then execs that exact candidate's private `install-service --darwin-candidate`. Before promotion, the candidate persists snapshot bytes and fsynced `MacDeploymentTransactionV1`; each promotion/pull/deployment/activation phase is durable. `CandidatePromotionAdmission` owns retained-byte promotion, `MacImageBootstrapAdmission` owns one exact arm64 pull in the only allowed absent-receipt state, and in-memory pull evidence derives `MacDeploymentCommitAdmission` for one rollback unit containing the exact receipt plus receipt-bound normalized config/plist bytes. Ordinary runtime admission then precedes launchd activation and runner work; private launchd startup validates the complete receipt and never pulls. A later candidate validates any nonterminal record and derives `MacDeploymentRecovery`, which can only prove completion or restore recorded bytes plus inactive launchd state before new bootstrap. Tests inject death after promotion and between every config/plist/receipt boundary, prove no second pull, and cover recovery denial/replay/mismatched-record zero effects; old/replaced/mismatched candidate zero effects; denial/expiry/revocation of all three bootstrap admissions; missing/mismatched snapshot/record remaining inactive; partial/stale/mismatched rejection; one-pull-only behavior; changed endpoint/plist/config/binary; child/local-ID substitution; valid restart with zero pulls; and zero launchd or Docker-runner effects on failure. Run shell syntax, `bash tests/runner_image_lock_test.sh`, generated-unit/service/alert tests, config/JIT exact-four-label tests, Rust lock-parser/platform tests, `cargo test`, `bash tests/assert_crash_containment_test.sh`, and `git diff --check`; then run `git add scripts/host/ezgha-start-gate.sh src/service.rs src/containment.rs src/config.rs src/main.rs src/alert.rs config/config.toml.linux.example config/config.toml.linux-canary.example config/config.toml.mac.example config/runner-image.lock tests/runner_image_lock_test.sh`, commit `codex/gpt-5.6-sol: wire contained service startup`, and push.
...
cargo test
bash tests/runner_image_publisher_test.sh
bash tests/runner_image_lock_test.sh
bash tests/assert_crash_containment_test.sh
git diff --check
```

Expected: all focused installer and assertion tests PASS with five already-pushed green checkpoints and no residual Task 4 diff.
...
- Create: `scripts/host/bootstrap-ezgha-sudoers.sh`
- Create: `scripts/host/bootstrap-docker-systemd-cgroup-driver.sh`
- Create: `systemd/host/ezgha-containment.sudoers.template`
...
- Create: `tests/bootstrap_docker_systemd_cgroup_driver_test.sh`
...
- Produces: one public atomic installation entrypoint, a tracked pre-deployment administrator bootstrap for the fixed privileged broker/sudoers/bundle-authorization contract, a separate empty-fleet Docker-driver bootstrap, shell-owned preflight/local-build/staging/dispatch ordering, explicit rehearsal/recovery-confirmation routing, timestamp-bounded complete pre/post job-outcome evidence, retirement of the conflicting watcher/exemption stack and every independently scheduled mutating queue reaper, a closed declarative effect registry, read-only topology/reaper diagnostics, and a CI-enforced hard harness gate. Task 5 owns `install.sh` command grammar and its exact `exec sudo -n` forms into Task 4's fixed privileged broker, including argument-free `rehearse-transaction`, `--recover-transaction --confirm-transaction <nonce>`, and receipt-independent/toolchain-independent `--clear-relief --confirm-request <id>`; it never owns quiescence, control locks, durable state, Rust handoff logic, or any live mutation primitive.
...
Create `tests/bootstrap_docker_systemd_cgroup_driver_test.sh` before the helper. Fixtures cover an already-correct read-only no-op; absent `daemon.json`; preservation of unrelated JSON keys; duplicate or conflicting `native.cgroupdriver` options; malformed JSON; an incompatible Docker `ExecStart` flag/config path; active `ezgha.service`, any managed container, or any live containment transaction; failed daemon restart/post-check with byte-exact rollback; and successful atomic replacement plus one restart and effective cgroup v2/systemd verification. The helper uses a structured JSON parser, fixed `/etc/docker/daemon.json`, safe non-symlink parent/file checks, and no caller-selected paths. It is an explicit administrator maintenance command only; it never runs from ordinary `install.sh`, never touches containment units, and never starts a runner or calls GitHub. Register it as `BootstrapPrivileged` with only daemon-config replacement/rollback and one Docker-service restart. The ordinary installer/preflight fixture must print this exact remediation when the driver is wrong while proving zero daemon-file or Docker-service effects.
...
Retire the old watcher stack atomically in this task. In the same commit: delete the four tracked watcher/exemption artifacts; remove their ordinary install and uninstall loops; add their exact final paths and prior state to the staging manifest so Task 4's Rust transaction disables/removes the installed timer, service, stable script, and live `~/.config/systemd/user/ezgha.service.d/10-oomd-omit.conf` through typed gates; verify `ManagedOOMPreference` is not `omit` and `OOMScoreAdjust` is not `-1000`; remove every legacy `systemctl set-property --runtime` path; update every active test and operator document; and replace Gate 7/8 watcher checks with exact production managed-OOM `auto` and finite-cgroup checks. Rewrite `docs/host-ops-sudo-block-0725.md` to name the watcher and exemption as retired and point to the tracked containment transaction. Add a regression scan covering `install.sh`, both doctors, queue scripts, `docs/verify-exit-criteria.sh`, `docs/host-ops-sudo-block-0725.md`, `docs/EXIT-CRITERIA.md`, `README.md`, `DESIGN.md`, `CLAUDE.md`, `GEMINI.md`, active tests, units, and workflows; fail when any active surface installs, enables, depends on, invokes, or suggests a retired watcher, exemption, reaper, runtime property unit, mutable runner image build, or retired queue/reaper shell apply/cancel path. The explicit `ManualAgentSessionMigration` class remains exempt. Historical activity/roadmap/goal records may retain incident references. Add a Task 5-owned forced-failure integration fixture that begins with byte-distinct installed watcher units/script and live exemption, plus active/enabled watcher state; inject failure after Rust retirement and prove exact file bytes, prior enabled/active states, and effective exemption properties are restored through Task 4 `ReliefGate`. No intermediate commit may delete a source artifact while the Rust transaction, verifier, or test still depends on it.

Extend `scripts/job_outcome_monitor.py` and `tests/job_outcome_monitor_test.py` with explicit UTC `--start`, optional `--end`, required `--output`, and `--require-complete-population`. The monitor atomically writes canonical JSON containing query bounds, eligible population, every included run/job identity and conclusion, threshold inputs, completeness reason, and verdict. Complete mode rejects truncated pagination, unknown conclusions, missing requested bounds, or an eligible population below `--minimum-jobs`; a post-activation artifact whose start precedes the receipt activation timestamp is invalid. Fixtures prove boundary inclusion/exclusion, atomic-output failure, incomplete population, and that disjoint pre/post ranges cannot share a job. Outcome evidence remains a release-health gate and makes no causal or duration claim.
...
Create `config/executable-effect-classes.toml` as the closed declarative inventory and `tests/crash_containment_effect_inventory_test.sh` as its exhaustive active-source/instruction gate. Inventory every tracked `.service`, `.timer`, `.path`, `.socket`, `.plist`, `.plist.template`, executable shell/Python file, workflow, doctor, installer, and active command/skill surface exactly once. Parse every systemd `ExecStart` and plist `ProgramArguments`, require one matching target record, reject orphan scheduled targets, and scan referenced executables as well as `install.sh`, both doctors, `scripts/**/*`, active `docs/*.sh`, `launchd/**/*`, `systemd/**/*`, `.github/workflows/*`, `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `DESIGN.md`, `README.md`, `config/README.md`, `.claude/commands/**/*.md`, and `.claude/skills/**/*.md`. The only closed classes are `RustGated`, `BootstrapPrivileged`, `ImmutableImagePublisher`, `ProofEffect`, `ReadOnly`, `CredentialRefresh`, `LocalRetentionCleanup`, `AlertDelivery`, `MacAuxiliaryManager`, `ManualAgentSessionMigration`, and `Retired`; each non-Rust record declares exact allowed targets and explicitly prohibits runner, Docker, GitHub-manager, service-start, VM-start, and host-lifecycle effects unless its class names that exact narrow operation. `BootstrapPrivileged` has exactly two disjoint named records: `BundleAuthorizationBootstrap` for `scripts/host/bootstrap-ezgha-sudoers.sh` and only its helper/authorization/sudoers safe-parent, atomic-rename, and rollback effects; and `DockerCgroupDriverBootstrap` for `scripts/host/bootstrap-docker-systemd-cgroup-driver.sh` and only structural `/etc/docker/daemon.json` replacement/rollback plus one `docker.service` restart after its empty-fleet proofs. The former forbids Docker; the latter forbids runner/GitHub/cgroup-policy/bundle/installed-service/VM/host-lifecycle effects; no path matches both.

Classify token refresh as `CredentialRefresh` limited to its own mode-`0600` token/cache atomic replacement; mission-output cleanup as `LocalRetentionCleanup` limited to declared owned output paths; fleet/Slack notification as `AlertDelivery`; and launchd installer, Colima trim, and dashboard publishing as `MacAuxiliaryManager`. That Mac class may own only its named non-Linux paths and must have no call edge from Linux install, doctors, CI, or the supervisor. Classify `.github/workflows/runner-image.yml` alone as `ImmutableImagePublisher`: its sole allowed effects are a GitHub-hosted Buildx build/push of the fixed two-platform OCI index `ghcr.io/jleechanorg/ezgha-runner:<source-sha>` and provenance-artifact upload from the exact workflow/Dockerfile/wrapper inputs. Its unique inventory record grants packages-only publication to that target and forbids self-hosted runners, Docker-socket mounts, production host/manager/VM/cgroup/runner-control access, workflow dispatch/cancel, repo-content writes, and configurable targets/contexts; zero, duplicate, broadened, or additional-platform records fail. Retire and delete `scripts/host/probe-9p-cgroup-charge.sh`, whose default 1-GiB allocation and `limactl` spawn are unnecessary for HostDocker containment, and prove no schedule, installer, CI, doctor, supervisor, or active-document edge remains. Fail on any unclassified GitHub write/dispatch/delete, Docker lifecycle verb, mutating `systemctl`/`launchctl`/VM command, `systemd-run`, cgroup-control write, privileged shell, or retired command suggestion. Replace active direct operational instructions, including `GEMINI.md`'s mutable runner build and raw cargo-install path, the config guide's transient `systemd-run ... serve`, `code-standards` raw cargo-install/restart/container removal, and `ezgha-install` Docker login/pull/VM/service mutations, with receipt-bound `./install.sh`/recovery, gated Rust canary/reaper/relief commands, or read-only inspection. Except for the declaratively classified hosted publisher and Rust's exact receipt-bound index pull, no shell/CI path builds, tags, pulls, or mutates a runner image, and CI contains no host-manager or Docker-socket mount. Historical activity/roadmap/goal records are excluded as non-executable history.
...
Add `docs/verify-exit-criteria.sh --require-attestation-calibration <artifact>`. Parse schema-1 JSON structurally and require current boot ID, receipt/config digests, captured workflow/run/attempt identity, exactly 100 consecutive successful full-load samples, rank-99 full-check duration at or below 5000 ms, maximum lag at or below 20000 ms, fast-guard rank-99 at or below 100 ms, ten passing capacity observations, and current receipt-bound assertion/provenance and pre-pull/READY records at or below 10/60 seconds. Reject unknown fields, duplicate sequences, stale boot/artifacts/run, tampering, insufficient samples, failed capacity, missing handoff timing, or any threshold failure. Fixtures are Task 5-owned and require this option to fail closed without invoking Docker, GitHub, systemd, or a live proof.
...
Make `install.sh` the single public deployment entrypoint and sole shell preflight/build/stage dispatcher; Task 4's Rust broker remains the sole owner of quiescence, locks, durable state, live commit/rollback ordering, and manager effects. Add `scripts/host/bootstrap-ezgha-sudoers.sh` as the sole interactive bundle-authorization/deployment privilege operation before a fresh deployment or any bundle/helper change. The Docker-driver helper is the only separate pre-install host-maintenance privilege operation and is unavailable once a transaction begins. The nonprivileged phase requires a clean exact build, successful static preflight, and a fully staged canonical bundle. The privileged bundle bootstrap verifies the deploy account/passwd home, descriptor-opens every entry without symlink traversal, computes canonical `DeploymentBundleAuthorizationV1`, and atomically installs `/usr/local/libexec/ezgha/ezgha-privileged` plus root-owned authorization metadata binding helper/broker protocol, bundle digest, every content/target/operation/metadata/expected-prior entry, and fixed inbox identity. It renders a user-specific `systemd/host/ezgha-containment.sudoers.template` as root-owned mode `0440`, validates it with `visudo -cf`, atomically installs it, and proves the deploy owner can use `sudo -n` for the four exact helper grammars. The target Ubuntu sudo version must pass a temp-root fixture for anchored command-argument regexes matching only `^transaction$`, `^rehearse-transaction$`, `^recover-transaction --confirm-transaction <strict-nonce>$`, and `^clear-relief --confirm-request <strict-request-id>$`. The sudoers rule uses `env_reset`, grants no `SETENV`, and authorizes no shell, extra argument, or caller-selected path; the helper independently repeats the strict grammar checks. Any bundle byte, target, operation, mode/owner, or expected-prior change after authorization fails before quiescence. Normal transactions cannot update the helper or authorization.

The executable-effect inventory classifies this script as the `BundleAuthorizationBootstrap` record of `BootstrapPrivileged`, a pre-quiescence administrator exception rather than ordinary deployment authority. Its allowlist is limited to the exact helper, root-owned metadata, temporary files in the same root-owned directories, and the one sudoers drop-in. It verifies every parent is root-owned, non-symlink, and not group/world writable; stages and validates all bytes first; uses no Docker, GitHub, cgroup, service-manager, runner, lock, or application-state operation; atomically renames only after all checks pass; and rolls back byte-exact prior files on any partial failure. Temp-root fixtures inject failure before and after every rename, verify full rollback, and prove all other paths/effects are rejected. Missing or invalid bootstrap state fails closed with the exact pre-quiescence repair instruction and never triggers an interactive prompt after quiescence.

Update `CLAUDE.md` Gate 0 so Linux deployment invokes this entrypoint and removes raw `cargo install --path .` plus direct restart as an accepted substitute. Remove the current config-exists auto-install/restart branch: a first pass with no config performs only `cargo build --locked --release` and prints `target/release/ezgha init`, static preflight/staging, tracked administrator bootstrap, and second-`./install.sh` steps. It does not replace an installed binary or build/pull/run a Docker image. Bootstrap `init` may query endpoint/daemon metadata read-only but cannot start/recover a backend; it writes only the mode-`0600` starter config and creates both the fixed canonical containment root and configured application `state_dir` as non-symlink directories owned by the deploy UID with exact mode `0700`. After the read-only static verdict and complete bundle staging into the fixed inbox, the administrator bootstrap authorizes that exact canonical bundle. The later `install.sh` requires exact manifest/bundle equality with root authorization, proves the fixed helper's `sudo -n` contract before quiescence, and ends with exactly `exec sudo -n -- /usr/local/libexec/ezgha/ezgha-privileged transaction`, so no shell coordinator or interactive prompt survives to race or delay it. It never uses `sudo -E`, arbitrary environment forwarding, a mutable sudo target, or a shell command string. The broker opens and validates the authorization, manifest, and every bundle entry by descriptor and runs the closed transaction itself; it installs candidate bytes only for later deploy-UID execution. Rust revalidates the static verdict, consumes any exact `RecoveryDecision`, and obtains the dynamic verdict before image or live mutation. `EZGHA_PROPOSED_CONFIG`, or the explicit config-only bootstrap path when that variable is absent, must resolve to a mode-`0600` regular proposed file; every install/handoff rejects a foreign-owned, symlinked, or group/world-writable containment or application-state directory. Config existence alone is staged data, not upgrade evidence; Rust uses the four-state classifier and rejects `Mixed` before Docker image or installed/live mutation. Fixtures begin from no config and prove the first pass records only the local Cargo build plus guidance, with zero service-manager, Docker, installed-binary, or live-config mutation.

Remove the current shell `docker build`/`docker tag`/mutable-`latest` block on both Darwin and Linux. `install.sh` builds only the Rust candidate and dispatches the applicable Rust install path. On Linux the broker owns the exact lock-bound index pull after fencing. On Darwin shell atomically stages only that candidate plus manifest in the fixed private inbox and execs its private self-promoting `install-service --darwin-candidate`; the candidate owns endpoint normalization, candidate promotion, lock parsing, `linux/arm64` child selection, the one gated index pull, platform/local-ID verification, receipt persistence, and launchd start. It cannot build, tag, retag, remove, pull another reference, alter Colima, or invoke Linux systemd policy. Installer and inventory fixtures reject every direct shell Docker image mutation and prove the six ARM64-labeled Mac runners use the verified receipt local ID.

Implement `scripts/host/install-crash-containment.sh` here with exactly `--preflight`, `--check`, and `--stage`; remove or fail closed on legacy `--install`. `--stage` writes only to the fixed private bundle inbox and emits the complete canonical manifest consumed by the bootstrap/broker. On Linux only, add a static shell scan plus execution traces proving the stager contains no `sudo`, and `install.sh` contains only the four exact final `exec sudo -n -- /usr/local/libexec/ezgha/ezgha-privileged ...` forms. Reject `sudo -E`, interactive sudo, a shell command string, any later sudo, a mutable sudo target, mutating `systemctl`, Docker image/runtime mutation, final-path write/remove, or rollback command in shell. Rehearsal dispatches argument-free `rehearse-transaction` and redirects only its stdout to a caller-owned evidence file; normal Linux install dispatches `transaction`; recovery dispatches `recover-transaction --confirm-transaction <nonce>`; clearance dispatches `clear-relief --confirm-request <id>` without Cargo, a checkout lookup, config, receipt, or bundle staging. Missing noninteractive privilege fails before quiescence, locks, or live mutation. On Darwin, `install.sh` has exactly one deployment dispatch after private staging: `exec <containment-root>/darwin-install-inbox/candidate install-service --darwin-candidate`. It supplies no caller-selected path or endpoint argument; the candidate derives and verifies its fixed inbox manifest plus normalized endpoint before promotion. Shell never stages a Linux broker bundle, invokes sudo/broker recovery grammars, or performs Docker, `launchctl`, Colima, installed-path, config, plist, or receipt mutation itself. Inventory tests validate the Linux and Darwin branches separately.
...
Once `install.sh` has `exec`d the fixed broker, the broker validates the complete root-authorized bundle and obtains the dynamic verdict; after both verdicts pass, it snapshots the installed binary bytes/path/mode/digest; live config; every old policy/helper file; the deployment receipt; exact generated `ezgha.service` and `ezgha-alert@.service` units; retired watcher/exemption files; and applicable active/enabled states. Before image or installed/live mutation, the broker verifies preserved `state_dir`/target/scope/prefix, acquires `emergency.lock` only to reject any relief latch, releases it, stops active `ezgha.service`, and verifies inactive/MainPID exit without either lock. It then reacquires `emergency.lock`, rechecks all latches, publishes `Preparing`, and releases it before bounded `serve.lock` acquisition and the `Fenced` transition. It never waits for service stop, process exit, or the fence while holding the control lock; any latch instead records `ReliefLatched`, restores no active service, and starts nothing. Managed-upgrade containers may remain only when their actual cgroups are already beneath `actions.slice`. Legacy migration drains under the broker-held FD without cancelling busy jobs. The broker validates numeric bounds/inode, requires `EWOULDBLOCK` on a second canonical descriptor, and never unlocks/closes the retained FD before terminal transfer or fail-closed recovery. Task 4 fixtures cover latch races, no control lock during waits, descriptor identity/contention, zero admission between fence and handoff, and release-build lock-bypass rejection; Task 5 fixtures prove exact bundle authorization, broker `exec` arguments, and absence of a surviving shell coordinator.

Only after quiescence, `Preparing`, fence acquisition, and nonce-matched `Fenced` may Rust pull the bundle-authorized immutable image digest and commit the already-verified bundle's candidate binary, mode-`0600` config, helpers, policy, both units, removals, and receipt manifest. The broker runs one exact digest-qualified `docker pull` as a gated deploy-UID child, verifies the requested repo digest and local image ID, records both in the receipt, and runtime creates containers from that ID. A mutable tag, wrong repo digest, or post-pull ID mismatch fails before runner work; no host path reaches Docker build context. Before its final `exec sudo -n`, the shell performs reads and private staging only: it never runs a mutating `systemctl`, performs a final-path write/remove, mutates Docker images/runtime, opens socket/peer state, or touches locks/latches. Task 4's root Rust broker revalidates `DeploymentBundleAuthorizationV1` and retained descriptors and routes every authorized image-pull/artifact/policy/unit commit/removal and system-manager operation separately through `DeploymentGate`; Docker and user-manager operations run only in explicitly downshifted deploy-UID/GID children with fixed receipt-bound environment. `TransactionAdmission` remains additionally required for deferred unit commits. The receipt enumerates the image repo digest/local ID plus all six desktop-boundary final paths and digests: four under `/etc/systemd/system/{-.slice.d,user.slice.d,user-.slice.d,user@.service.d}` and two under `/etc/systemd/user/{app.slice.d,session.slice.d}`. Each image pull, artifact commit/remove, and system/user daemon reload is a separate typed effect; rollback restores exact prior bytes only through `ReliefGate`, then performs the matching reload. A newly cached immutable image may remain as inert content because runtime cannot use it without the receipt-bound ID. Config and receipt artifacts are one Rust-owned rollback unit. Rust checks durable relief before/after each bounded operation, before deferred install, at least once per second during drain, and through handoff. Latch acknowledgment atomically records `ReliefLatched`, blocks every later forward gate, rolls back under `serve.lock` through `ReliefGate`, and leaves the supervisor inactive. Stale/malformed/mismatched transaction or latch state remains fail-closed until exact verified recovery/clearance; shell never retries an unknown live result.

Implement two distinct recovery roles in `install.sh`. The shell validates mutually exclusive exact syntax and performs no lock, latch, state, socket, service-manager, or live-file mutation itself. Non-emergency `./install.sh --recover-transaction --confirm-transaction <nonce>` ends with `exec sudo -n -- /usr/local/libexec/ezgha/ezgha-privileged recover-transaction --confirm-transaction <nonce>`; the broker validates its own root-owned digest/metadata and the exact receipt-bound installed-binary identity, then runs the closed recovery state machine itself without root-executing that binary. A missing or path/mode/digest-mismatched helper, metadata, receipt, or binary, or unavailable noninteractive privilege, fails before recovery. Rust then acquires `serve.lock` without holding `emergency.lock`, rejects unresolved requests, requires the service inactive, proves no live transaction/service owner, and accepts only matching `Preparing`, `Fenced`, `HandoffReserved`, `HandoffClaimed`, `HandoffLocked`, `SubmissionUnknown`, or `RecoveryRequired`. It completes/verifies saved rollback or proves no mutation, removes only identity-proven stale socket state, fsyncs `RecoveredInactive`, and leaves pre-baton state disabled/inactive while permitting a later clean transaction retry.

Relief `./install.sh --clear-relief --confirm-request <id>` performs no build, checkout lookup, config read, receipt validation, or candidate staging. It validates only the request-ID grammar, then ends with `exec sudo -n -- /usr/local/libexec/ezgha/ezgha-privileged clear-relief --confirm-request <id>`. The already-installed broker validates its root-owned path/mode/digest and metadata, derives the fixed canonical containment root from the deploy UID's passwd home, and may open only that root, `serve.lock`, `emergency.lock`, transaction ownership records, and the request journal. It requires the service inactive, nonblocking exclusive `serve.lock` acquisition, no live owner, and the exact request ID/digest, then tombstones only that request and fsyncs the collection. It cannot build, read config or receipts, open transaction snapshots or deployment artifacts, invoke Docker/GitHub, start/reload/restore a manager, or accept arbitrary path, command, or environment input. Other requests remain durable. Clearing the final latch does not make a damaged or `Mixed` deployment admissible; ordinary startup and receipt-bound recovery still fail their own full checks. A missing/invalid broker fails closed and is repaired only by the tracked administrator bootstrap; it never falls back to Cargo or a mutable checkout. Fixtures cover missing/malformed/replaced receipts, no config, bootstrap/pre-baton state, a non-default application `state_dir`, wrong ID, repeated requests, remaining-request blocking, active service/live canonical-lock denial, dead-owner exact tombstone, helper replacement, exact `sudo -n` argv, zero manager/backend/GitHub/deployment effects, toolchain absence, and startup remaining blocked after clearance when artifacts are invalid.

After `install.sh` `exec`s the fixed broker, Task 4's broker-owned Rust transaction internally invokes deferred unit installation, final admission, and `handoff-service`; it alone owns the transaction FD, typed state, effect gates, wait/deadline handling, and rollback. Task 5 ends at exact fixed-helper dispatch. Its integration fixtures prove complete canonical bundle staging, the four exact helper argv grammars, effect-free canonical rehearsal stdout, `BootstrapPrivileged` path/effect/rollback limits, sudoers validation, root-owned bundle authorization, retained descriptor identity, exit-status propagation, toolchain-independent clearance, and no surviving shell or shell-side live effect. Substitution fixtures pair an authorized candidate with changed config, generated unit, policy, helper, image lock/reference/source/input/provenance, target, operation, mode/owner/expected-prior state, duplicate/missing/extra manifest entry, manifest rewrite, and artifact pathname replacement; every mismatch proves zero lock/state/gate/Docker/backend/manager/final-path effect, while a positive pathname-race fixture commits only previously verified retained bytes. Image fixtures prove host build/tag/network paths are forbidden, the pull reference is digest-qualified, wrong pulled/runtime digest fails, runtime uses the receipt-bound local ID, and no checkout path is passed to Docker. FD preservation, `READY`, typed handoff outcomes, rollback, peer proof, `SCM_RIGHTS`, close-on-exec, socket cleanup, terminal-owner CAS, death windows, and deadline boundaries are Task 4 Rust tests committed with its Rust files. Receipt parser fixtures also prove missing/wrong repo digest, local image ID, lock digest, or publisher provenance and post-pull image substitution fail before runner work, while rollback restores the prior receipt/config atomically and leaves any newly cached immutable image inert.
...
- [ ] **Step 5: Land four serial green Task 5 checkpoints**

The Task 5 inventory contains 61 paths, including the Docker-driver bootstrap pair, job-outcome monitor pair, and active `GEMINI.md` and `DESIGN.md`; the runner-image publisher and lock are already green in Task 4D/4E. Task 4 must be fully green first, and no live deployment or pressure proof runs between these checkpoints. The disjoint partition is 5A=10, 5B=39, 5C=6, and 5D=6.

Checkpoint 5A owns only policy and operator documentation:

```bash
bash tests/crash_containment_policy_test.sh
rg -n 'Crash-containment contract|Backend choice is subordinate|reset-failed ezgha.service|staged bootstrap state' CLAUDE.md AGENTS.md README.md config/README.md
git diff --check
git add CLAUDE.md AGENTS.md GEMINI.md DESIGN.md README.md config/README.md \
  .claude/commands/doctor-ezactions.md .claude/commands/code-standards.md \
  .claude/skills/ezgha-doctor/SKILL.md .claude/skills/ezgha-install/SKILL.md
git commit -m "codex/gpt-5.6-sol: align runner containment policy"
git push origin HEAD
```

Checkpoint 5B atomically owns `install.sh`, bootstrap/staging, all retirement-dependent consumers, and all 13 legacy deletions. No surviving installer or fixture may reference a deleted effect:

```bash
bash -n install.sh scripts/host/install-crash-containment.sh scripts/host/bootstrap-ezgha-sudoers.sh \
  scripts/host/bootstrap-docker-systemd-cgroup-driver.sh \
  scripts/cleanup-stuck-runs.sh scripts/queue-backlog-drain.sh scripts/queue-health.sh \
  docs/test-cleanup-stuck-runs.sh docs/test-queue-backlog-drain.sh docs/verify-exit-criteria.sh
bash tests/install_crash_containment_test.sh
bash tests/bootstrap_docker_systemd_cgroup_driver_test.sh
...
bash tests/assert_qemu_cpu_ceiling_test.sh
...
git diff --check
git add install.sh scripts/host/install-crash-containment.sh scripts/host/bootstrap-ezgha-sudoers.sh \
  scripts/host/bootstrap-docker-systemd-cgroup-driver.sh \
  systemd/host/ezgha-containment.sudoers.template scripts/cleanup-stuck-runs.sh \
  scripts/queue-backlog-drain.sh scripts/queue-health.sh docs/host-ops-sudo-block-0725.md \
  docs/test-cleanup-stuck-runs.sh docs/test-queue-backlog-drain.sh docs/verify-exit-criteria.sh \
  scripts/host/assert-qemu-cpu-ceiling.sh \
  tests/assert_qemu_cpu_ceiling_test.sh tests/cleanup_stuck_runs_force_cancel_test.sh \
  tests/cleanup_stuck_runs_multirepo_test.sh tests/cleanup_stuck_runs_race_skip_test.sh \
  tests/cleanup_stuck_runs_verify_inconclusive_test.sh tests/install_watchdog_gate_test.sh \
  tests/install_uninstall_aux_units_test.sh tests/install_transaction_recovery_test.sh \
  tests/install_crash_containment_test.sh tests/bootstrap_docker_systemd_cgroup_driver_test.sh \
  tests/host_control_artifacts_test.sh tests/host_ops_0725_test.sh \
  tests/verify_exit_gate8_test.sh
git add -u systemd/ezgha.service.d/10-oomd-omit.conf systemd/psi-oom-watcher.service \
  systemd/psi-oom-watcher.timer scripts/host/psi-oom-watcher.sh \
  systemd/ezgha-queue-reaper.service systemd/ezgha-queue-reaper.timer \
  scripts/queue-reaper-stopgap.sh launchd/org.jleechanorg.ezgha-queue-reaper-stopgap.plist.template \
  systemd/lima-vm-cpu-ceiling.service systemd/agent-scope-reaper.service \
  systemd/agent-scope-reaper.timer scripts/host/agent-scope-reaper.sh scripts/host/probe-9p-cgroup-charge.sh
git commit -m "codex/gpt-5.6-sol: retire competing runner effect paths"
git push origin HEAD
```

Checkpoint 5C owns doctors, exit-criteria documentation, and topology consumers after 5B has atomically rewritten and tested the Gate 8 verifier with watcher retirement:

```bash
bash -n doctor.sh doctor-runner
bash tests/doctor_runner_host_pressure_test.sh
bash tests/doctor_canary_admission_test.sh
bash tests/topology_golden_consumers_test.sh
git diff --check
git add doctor.sh doctor-runner docs/EXIT-CRITERIA.md \
  tests/doctor_runner_host_pressure_test.sh \
  tests/doctor_canary_admission_test.sh tests/topology_golden_consumers_test.sh
git commit -m "codex/gpt-5.6-sol: gate doctor containment evidence"
git push origin HEAD
```

Checkpoint 5D owns the closed effect inventory and CI harness, and runs last against the completed active tree:

It adds both Task 4 image tests to `tests/crash_containment_suite.sh`'s deterministic explicit list; `.github/workflows/ci.yml` continues to invoke that suite. The inventory fixture requires exactly one `ImmutableImagePublisher` record for `.github/workflows/runner-image.yml` and rejects every missing, duplicate, broadened, or additional workflow Docker/package-write effect.
...
git diff --check
git add config/executable-effect-classes.toml scripts/job_outcome_monitor.py \
  tests/job_outcome_monitor_test.py tests/crash_containment_effect_inventory_test.sh \
  tests/crash_containment_suite.sh .github/workflows/ci.yml
git commit -m "codex/gpt-5.6-sol: enforce executable effect inventory"
git push origin HEAD
```

Expected: every checkpoint is green before its exact add/commit, and Task 5 leaves no residual diff.
...
- Produces: a caller-selected, repository-relative, unique evidence directory under `evidence/crash-containment/<timestamp>/` with separate `hard-limit`, `oomd-selection`, `ten-worker`, `attestation-calibration`, and `survival` verdicts. Every live mode requires `--evidence-dir <path>`, rejects an absolute/out-of-tree/symlinked path, and appends only its own fixed artifact names so Task 7's earlier baseline and rehearsal remain in the same release directory.
...
calibration requested without both explicit opt-ins                  => FAIL before dispatch
calibration uses any hold other than exact 2700 seconds              => FAIL before dispatch
100 production observations remain bound to 10 captured workers      => attestation-calibration PASS
collector gap/failure or one capacity sample loses a worker          => calibration FAIL without cancellation
```
...
1. Require `EZGHA_RUN_PRESSURE_PROOF=1` plus one safe `--evidence-dir` beneath the repository's fixed evidence root, reject `CI` or any recognized CI environment before mutation, run the read-only containment assertion, require `/ezghaproof.slice` and both proof children to contain no process, and require every production runner path to remain below `/actions.slice`, outside the proof subtree. Disabled, CI, unsafe evidence path, or failed-preflight paths produce zero proof effects.
...
8. Change the existing capacity-proof matrix to exact ordered selector `C = [self-hosted, self-hosted-mikey, ezgha, ez-runner-c]`; repository fixtures prove the Mac and canary examples do not contain `ez-runner-c`, and no `Linux`/`X64` supplement reaches config, JIT argv, `runs_on_json`, or the workflow selector. The only accepted `workflow_dispatch` hold input is 240 seconds for ordinary ten-worker proof or 2700 seconds for explicit attestation calibration. Before calling `C` fleet-unique or dispatching, query every runner in the actual GitHub registration scope and preserve every label `{name,type}` object as read-only inventory. Match runners by `C` as a subset of response names; require the matching IDs/names to be exactly `ez-runner-c-1..10` and fail on any eleventh match regardless of supplemental read-only labels. Apply strict type/custom-set validation only to those `C` matches: each must have custom-label set exactly `C - {self-hosted}` and only optional additional `type: read-only` labels. Unrelated nonmatching Mac, canary, or sibling runners, including custom-labeled ones, remain inventory evidence and do not fail the query. For a `C` match, missing/unknown types, duplicate/empty names, missing `C`, or an unexpected custom label fail closed. Record the full collision-query result and remote supplements without feeding them back into routing. Fixtures cover ten expected runners with read-only `self-hosted`/`Linux`/`X64` supplements, an unrelated custom-labeled nonmatch that passes as inventory, an extra matching eleventh runner, missing `C`, unexpected custom label, duplicate/empty/unknown labels, and reordered objects. Replace the separate hold step with one shell step that writes `/tmp/ezgha-capacity-proof.marker` containing its `GITHUB_RUN_ID`, `GITHUB_RUN_ATTEMPT`, matrix number, and selected hold, installs an exit trap, sleeps for exactly that validated hold while the marker remains present, and removes it on that same shell's exit. The proof fixture fails unless the exact selector, registry-wide collision check, bounded hold contract, and same-step marker/hold contract are present. Snapshot recent capacity run IDs, dispatch with the exact validated input, and resolve exactly one new run ID. Poll direct host Docker until ten distinct managed containers simultaneously show `Runner.Worker`; require each in-container marker to bind the captured run/attempt and ten distinct matrix numbers; record each actual PID cgroup path beneath `/actions.slice`; and fail on timeout, fewer than ten, or any missing/mismatched marker. Watch only the captured run ID to successful completion and require its job metadata to name those same matrix jobs; never cancel it, because cancellation would contaminate job-outcome and calibration evidence. Bind evidence to the exact run ID, attempt, workflow SHA, ref, selector, hold, registration-scope runner IDs/names/typed labels, container IDs, markers, process samples, job metadata, and timestamps.

9. Add `--calibrate-attestation` to `--ten-workers`. It requires both `EZGHA_RUN_PRESSURE_PROOF=1` and `EZGHA_RUN_ATTESTATION_CALIBRATION=1`, selects only the exact 2700-second hold, and invokes Task 4's `containment-calibrate --observe-only --samples 100` after ten captured workers are present. The 2700-second bound covers 100 real production cycles at worst compliant `20s + 5s` cadence plus startup slack; burst sampling is forbidden. The collector itself performs and records the narrow read-only ten-worker marker/cgroup observation keyed to every consumed attestation sequence; the proof harness supplies the immutable captured-run identity, writes the resulting `attestation-calibration.json` in the evidence directory, and then watches the run normally. Collector failure never cancels the workflow or running jobs. CI exercises stubs only and never dispatches calibration.

- [ ] **Step 4: Add an opt-in harness gate**

Use the Task 5 verifier's existing proof-result interface and gate live execution behind `EZGHA_RUN_PRESSURE_PROOF=1`; calibration additionally requires `EZGHA_RUN_ATTESTATION_CALIBRATION=1`. The normal harness reports each live verdict as `NOT RUN (explicit live proof required)` rather than claiming containment from fixtures or configuration. A release/deployment acceptance run requires both pressure modes, the calibration-backed ten-runner proof, and survival observation. Do not modify `docs/verify-exit-criteria.sh` in this task.
...
This task is the only Linux live activation. It requires all prior checkpoints, the hosted multi-platform image artifact, lock, broker authorization, complete pre-activation outcome evidence, a matching read-only production-state rehearsal, and non-live suites green before quiescence. The service remains inactive throughout drain/cutover; no partial policy or legacy supervisor is activated. Failure restores only a complete receipt-proven prior deployment through the documented handoff or remains disabled/inactive `RecoveryRequired`. It does not invoke `launchctl`, `install-service --darwin-candidate`, or any Mac Docker/Colima/image/config/plist mutation; the six complete legacy Mac runners remain untouched until Task 8.
...
Create the evidence directory before any prerequisite or release-health capture:

```bash
EVIDENCE_DIR="evidence/crash-containment/$(date -u +%Y%m%dT%H%M%SZ)"
mkdir -p "$EVIDENCE_DIR"
docker info --format 'cgroup_version={{.CgroupVersion}} cgroup_driver={{.CgroupDriver}}'
```

Ordinary Task 7 proceeds only if the already-running daemon reports exactly cgroup v2 plus `systemd`. On a fresh host, or in a separately scheduled maintenance window after proving `ezgha.service` inactive, zero `ezgha=managed` containers, and no containment transaction, the administrator may run `sudo -- ./scripts/host/bootstrap-docker-systemd-cgroup-driver.sh`. The helper records before/after daemon metadata, structurally preserves unrelated `/etc/docker/daemon.json` keys, restarts Docker once, and rolls back its bytes on failed verification. A driver mismatch on a live fleet aborts Task 7; the helper is never smuggled into the activation transaction. Its explicit blast radius is interruption of non-managed Docker workloads during that separate restart.

Capture a complete baseline ending before any stage/bootstrap/fence operation:

```bash
BASELINE_END="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
BASELINE_START="$(date -u -d '24 hours ago' +%Y-%m-%dT%H:%M:%SZ)"
python3 scripts/job_outcome_monitor.py \
  --repo jleechanorg/ez-gh-actions \
  --start "$BASELINE_START" --end "$BASELINE_END" \
  --minimum-jobs 10 --minimum-success-rate 0.90 --sample-target 20 \
  --require-complete-population \
  --output "$EVIDENCE_DIR/job-outcome-pre-activation.json"
```

Expected: Docker is already v2/systemd and the baseline verdict is `HEALTHY`. `UNKNOWN`, `UNHEALTHY`, truncated pagination, unknown conclusions, or fewer than ten eligible jobs stops the release before live mutation. This baseline is a control-arm health signal, not a causal claim.

For the existing Jeff-Ubuntu legacy migration, first parse `config/runner-image.lock` with Task 4's Rust lock parser. Require one immutable `ghcr.io/jleechanorg/ezgha-runner@sha256:<64-hex>` reference and valid publisher source/workflow/Dockerfile/context provenance. Record the live config SHA-256, parsed `runner.image`, and whether that exact locked digest is already locally present using read-only inspection; preparation may not pull, build, tag, remove, or otherwise mutate Docker. Require the legacy prior image to be exactly `ezgha-runner:latest`; an existing digest-qualified or any other prior reference is an upgrade/profile-review path and stops here. Then have the Task 4 configuration serializer create a mode-`0600` temporary `PROPOSED_CONFIG` from `~/.config/ezgha/config.toml` with:
...
image = "<exact ghcr.io/jleechanorg/ezgha-runner@sha256 digest from config/runner-image.lock>"
...
Remove VM-only `vm_total_mb`, `guest_reserve_mb`, and `host_reserve_mb` from the proposed host-Docker profile. The serializer rejects duplicate keys and proves parsed equality of every other configuration field, including credentials and alert settings; it does not expose those values. The only reviewed legacy deltas are the retired VM fields, the listed containment keys, and `runner.image = "ezgha-runner:latest"` changing to the exact lock reference. Any other delta fails before staging, and this step never replaces the live config.

Before preparing the proposed config, record the existing effective Docker endpoint plus `runner.count`, `runner_floor_mb`, `runner.labels`, `runner.image`, and `limits.memory_mb`. The confirmed Jeff-Ubuntu values are the canonical host socket, `10`, `2500`, `[self-hosted, self-hosted-mikey, ezgha, ez-runner-c]`, mutable legacy `ezgha-runner:latest`, and `2500`. The reviewed image blast radius is explicit: all existing legacy containers keep their current image until the fenced drain, the broker later performs one receipt-authorized digest pull, and only subsequent runner creation uses its verified local ID; no image is rebuilt, retagged, or replaced in place. Any endpoint change, label removal/addition, higher memory value, count change, or image transition other than legacy `latest` to the exact lock digest stops for profile review. The transaction in Step 4 owns live replacement and rollback.

On a fresh host with no config, run `./install.sh` once to build only `target/release/ezgha`, run `target/release/ezgha init --target owner/repo` to create the explicit-endpoint mode-`0600` starter config, run static preflight and stage the complete canonical bundle, run the tracked administrator bootstrap to install the broker and authorize that exact bundle, then rerun `./install.sh`. The config-only state is `Bootstrap`, not `ManagedUpgrade`; it may be used directly as the proposed file because no receipt, generated unit, service, container, or registered prefix exists. The candidate binary is installed only inside the second-pass transaction and is never root-executed. Any other partial artifact combination is `Mixed` and stops for diagnosis.
...
Proceed only when the preflight exits zero before mutation, Docker reports cgroup v2/systemd, unprivileged `oomctl` works, and the classified state matches the intended deployment. For the first Jeff-Ubuntu `LegacyMigration`, require load at most 12, exactly all ten expected managed legacy containers, zero local `Runner.Worker` processes, no target cgroup under pressure, and sufficient current-use margins for any already-existing cgroups; there is no 8-of-10 or low-load exception. `ManagedUpgrade` requires every runtime cgroup plus the Task 4 margins: 1 GiB for actions, 1 GiB for agents, and 512 MiB for automation. `Bootstrap` permits absent not-yet-installed cgroups but no runtime/deployment artifacts. Record checksums of the current binary, policy, helpers, and both generated user units plus active/enabled state for rollback evidence. Any failed prerequisite stops here without changing live state. Preflight also verifies the captured prior config SHA/image, proposed mode and exact allowed-delta proof, equality of proposed `runner.image` with the tracked lock, and lock schema/provenance. It records the prior local `ezgha-runner:latest` image ID when present but treats it only as rollback/cache evidence and never pulls the new digest before fencing.

After that read-only preflight passes, run `scripts/host/install-crash-containment.sh --stage --config "$PROPOSED_CONFIG"` to build the complete canonical bundle without privilege or live effects. Before quiescence, run `scripts/host/bootstrap-ezgha-sudoers.sh` from the clean reviewed tree. It asks for administrator privilege only in this step, descriptor-verifies every staged entry, and atomically installs the root-owned broker/sudoers/`DeploymentBundleAuthorizationV1`. Rerun the read-only preflight and require exact canonical manifest/bundle equality with that root-owned authorization. Any source, lockfile, feature, content, target, operation, mode/owner, or expected-prior change after authorization returns to staging and bootstrap; deployment never silently reauthorizes.

Then exercise the actual authorized production parser/classifier/planner through the fixed broker without giving it an effect-capable coordinator:

```bash
REHEARSAL="$EVIDENCE_DIR/legacy-migration-rehearsal.json"
./install.sh --rehearse-transaction >"$REHEARSAL"
jq -e '
  .schema == 1 and
  .classification == "LegacyMigration" and
  .effect_capability_present == false and
  .managed_container_count == 10 and
  .worker_count == 0 and
  (.managed_container_names | sort) ==
    ["ez-runner-c-1","ez-runner-c-10","ez-runner-c-2","ez-runner-c-3","ez-runner-c-4","ez-runner-c-5","ez-runner-c-6","ez-runner-c-7","ez-runner-c-8","ez-runner-c-9"] and
  ((now - (.idle_proof_time | fromdateiso8601)) >= 0) and
  ((now - (.idle_proof_time | fromdateiso8601)) <= 300)
' "$REHEARSAL"
```

Expected: `LegacyMigrationRehearsalV1` binds the current boot, config, broker, authorization, live service/container fingerprints, exact planned effect sequence, rollback space, and timing bounds, is at most five minutes old, and reports no effect capability. It proves exactly the ten expected container names/IDs and `worker_count=0`; zero, fewer, extra, duplicate, or renamed containers fail. Any busy worker, rehearsal drift, expired artifact, or state mismatch stops with zero mutation. The live broker independently recomputes the same exact ten-container/zero-worker planner inputs and rejects drift before its first removal; the evidence artifact is not a serialized capability and supplies no caller-controlled path to the broker.
...
Expected: `install.sh` obtains the static verdict, requires the staged canonical manifest and every descriptor to equal root-owned bundle authorization, recomputes the rehearsed planner inputs, and `exec`s the fixed broker before any live mutation. The broker never executes the candidate as root; it obtains the dynamic verdict, snapshots all artifacts, checks the relief latch under `emergency.lock`, releases that lock before stopping/waiting for the old supervisor, then reacquires/rechecks it to publish `Preparing`. Rust acquires canonical `serve.lock`, drains legacy containers without cancelling jobs, and commits each authorized retained artifact/removal through `DeploymentGate` without starting the candidate prematurely. The zero-busy proof minimizes expected downtime; if a job races the fence, the busy-job-preserving `DrainDeadline` is 1800 seconds. Final admission validates every receipt artifact; the Rust handoff helper queues exact private mode for deploy-UID execution while fenced, proves the claimed systemd peer, and transfers the continuously locked FD with `SCM_RIGHTS`/receiver close-on-exec inside the fixed connect/ready deadlines. The claimant sends `FD_ACCEPTED` before the transaction closes its copy, then proves listener closure plus matching socket cleanup before recording boot-stamped `ServiceOwned`, pre-pull, `READY=1`, or runner work. Ordinary restart resolves and archives only a proven dead same/prior-boot owner after taking the lock; live/malformed evidence fails closed. Failure before ownership stops candidate, verifies transferred-FD closure, reacquires, and restores through `ReliefGate`. Compatible rollback repeats descriptor handoff; pre-baton rollback remains disabled/inactive and the broker-owned receipt-bound recovery state machine makes it retryable without starting it. Blast radius and rollback are fixed: all ten legacy containers drain before the broker's one receipt-authorized digest pull, and subsequent runner creation uses only the receipt-bound local ID. The receipt records capacity unavailable from first legacy removal until exactly ten new local managed containers are proven beneath `/actions.slice`. `ServiceReadyDeadline=210s` begins after zero legacy containers; `FleetConvergenceDeadline=600s` begins at `READY=1`; and the cumulative bound from first removal is `1800+210+600=2610s`. Exactly ten containers at the boundary passes. Fewer than ten or one millisecond over any deadline marks a failed cutover/release and records the incident; it is never reported as successful containment. A failed pre-baton cutover has no automatic runner-availability bound: all ten Linux slots may remain down in `RecoveryRequired` until operator recovery, and the release must report that incident rather than restart the uncontained legacy fleet. No container is retagged, rebuilt, or replaced in place. Before `ServiceOwned`, failure removes only transaction-created runtime resources, restores captured prior config/artifacts through `ReliefGate`, preserves the old local `latest` image only as inert cache, and leaves the legacy mutable-tag service disabled/inactive in `RecoveryRequired`; it never auto-restarts that fleet. After ownership, rollback restores the receipt-defined prior deployment unit atomically or remains `Mixed`/`RecoveryRequired` on ambiguity. Fake-clock fixtures pin all three deadline boundaries, ten-versus-fewer convergence, and the 2610-second total. Fixtures require the legacy-latest-to-lock-digest change to be the sole image delta; wrong/mutable/missing locks, duplicate image keys, other config changes, or pre-fence pull/build/tag produce zero live effects, and runtime container argv must use the receipt local ID rather than `latest`.
...
```bash
EZGHA_RUN_PRESSURE_PROOF=1 EZGHA_RUN_ATTESTATION_CALIBRATION=1 \
  bash scripts/host/prove-crash-containment.sh --ten-workers --calibrate-attestation \
  --ref "$(git branch --show-current)" --evidence-dir "$EVIDENCE_DIR"
./docs/verify-exit-criteria.sh --require-attestation-calibration \
  "$EVIDENCE_DIR/attestation-calibration.json"
EZGHA_RUN_PRESSURE_PROOF=1 bash scripts/host/prove-crash-containment.sh \
  --hard-limit --evidence-dir "$EVIDENCE_DIR"
EZGHA_RUN_PRESSURE_PROOF=1 bash scripts/host/prove-crash-containment.sh \
  --oomd --evidence-dir "$EVIDENCE_DIR"
./docs/verify-exit-criteria.sh
ACTIVATION_TIME="$(jq -r '.activation_time' "$EVIDENCE_DIR/deployment-receipt.json")"
python3 scripts/job_outcome_monitor.py \
  --repo jleechanorg/ez-gh-actions \
  --start "$ACTIVATION_TIME" \
  --minimum-jobs 10 \
  --minimum-success-rate 0.90 \
  --sample-target 20 \
  --require-complete-population \
  --output "$EVIDENCE_DIR/job-outcome-post-activation.json"
```

Expected: calibration mode dispatches exactly one captured 2700-second `capacity-proof.yml` run, records ten simultaneous managed containers whose direct `docker top` output includes `Runner.Worker`, correlates each container to the captured run/attempt through its exact marker and unique matrix number, proves actual cgroups are beneath `/actions.slice` at every one of 100 consecutive production attestation completions, and watches that run to success without cancellation. The verifier accepts only the current boot/receipt/config/run-bound schema-1 artifact, rank-99/lag/fast-guard thresholds, and current handoff timing records. The hard-limit proof records a local cgroup OOM; the oomd proof records both child pressure above threshold and a `systemd-oomd` victim beneath `ezghaproof-oomd.slice`; boot ID, user-manager PID, Warp, Docker, and unrelated runners survive both; the host-Docker Gate 8 passes without invoking Lima; and both disjoint job-outcome artifacts report `HEALTHY`, with the post population beginning no earlier than the receipt activation timestamp. No undefined duration-regression or causal claim is made because the 2000% aggregate quota equals the ten configured 2-CPU maxima.
...
Keep `EVIDENCE_DIR` set to the single directory created before baseline capture, then run:
...
git add CLAUDE.md AGENTS.md README.md roadmap/up-changelog-20260831-crash-containment.md \
  ...
  scripts/host/bootstrap-ezgha-sudoers.sh \
  scripts/host/bootstrap-docker-systemd-cgroup-driver.sh \
  scripts/host/prove-crash-containment.sh \
  systemd/host/ezgha-containment.sudoers.template \
  install.sh .github/workflows/runner-image.yml config/runner-image.lock scripts/cleanup-stuck-runs.sh scripts/queue-backlog-drain.sh scripts/queue-health.sh scripts/job_outcome_monitor.py config/README.md config/config.toml.linux.example config/config.toml.linux-canary.example config/config.toml.mac.example config/executable-effect-classes.toml doctor.sh doctor-runner \
  ...
  tests/host_crash_containment_artifacts_test.sh \
  tests/assert_crash_containment_test.sh \
  tests/runner_image_publisher_test.sh \
  tests/runner_image_lock_test.sh \
  tests/install_crash_containment_test.sh \
  tests/bootstrap_docker_systemd_cgroup_driver_test.sh \
  tests/job_outcome_monitor_test.py \
  tests/install_transaction_recovery_test.sh \
  ...
  GEMINI.md DESIGN.md .github/workflows/capacity-proof.yml "$EVIDENCE_DIR"
...
### Task 8: Deploy and Prove the Six-Mac Receipt Transaction

**Files:**
- Deployment-only: `~/.config/ezgha/config.toml`
- Deployment-only: `~/.local/libexec/ezgha/`
- Deployment-only: `~/Library/LaunchAgents/org.jleechanorg.ezgha.plist`
- Deployment-only: the fixed Darwin containment root, transaction snapshot/record, and `MacImageReceiptV1`
- Evidence: `evidence/crash-containment-mac/<timestamp>/`

**Interfaces:**
- Consumes: the exact green source SHA from Task 7, Task 4's Darwin transaction tests, the two-platform image lock, and one complete six-runner legacy Mac deployment.
- Produces: one independently atomic receipt-bound `linux/arm64` Mac deployment, six locally proven Mac runner slots, zero-pull restart evidence, and durable rollback/recovery evidence. It has no Linux systemd, Docker-daemon, host-policy, or Task 7 receipt effect.

This is a separate release boundary owned by the Mac deploy owner and never overlaps Task 7. Before this task, the accepted fleet state is ten contained Linux runners plus six complete legacy Mac runners. After it, the accepted state is ten contained Linux runners plus six receipt-bound Mac runners. A mixture of legacy and receipt-bound Mac artifacts or fewer than six Mac slots is not an accepted state.

- [ ] **Step 1: Verify source and transaction fixtures without live effects**
...
Expected: the exact Task 7 source SHA is checked out and clean; the lock is one OCI index with exactly `linux/amd64` and `linux/arm64`; death/recovery fixtures pass at every `Prepared -> CandidatePromoted -> ImagePulled -> DeploymentCommitted -> Activated` boundary; and startup fixtures prove missing/stale/mismatched receipts cause zero pull, launchd, or runner effects.

- [ ] **Step 2: Capture and validate the complete legacy Mac state**

Create a unique evidence directory and record config/binary/plist digests, launchd state, normalized endpoint, local image identity, and all six `ez-mac-runner-b-1..6` containers. Require safe non-symlink deploy-UID-owned mode-`0700` inbox/containment parents, a six-slot Mac config, the exact lock reference, and the `linux/arm64` child. Any partial Mac transaction/receipt, missing slot, endpoint disagreement, active recovery record, or source drift stops before staging or Docker mutation.

- [ ] **Step 3: Run the private Darwin candidate transaction**

```bash
./install.sh
launchctl print "gui/$(id -u)/org.jleechanorg.ezgha"
```

Expected: shell performs private inbox staging and then exactly `exec <fixed-inbox>/candidate install-service --darwin-candidate`. The candidate validates its manifest and durable state, promotes retained bytes, performs exactly one digest-qualified pull of the locked `linux/arm64` child under `MacImageBootstrapAdmission`, commits the exact binary/config/plist/`MacImageReceiptV1` set under `MacDeploymentCommitAdmission`, and only then activates launchd under ordinary `RuntimeAdmission`. It never builds, tags, retags, removes an image, starts/stops Colima outside the already authorized endpoint recovery, or touches Linux policy.

- [ ] **Step 4: Prove receipt identity, six-runner capacity, and zero-pull restart**

Require the receipt to match the installed binary, normalized endpoint, config, plist, OCI index, `linux/arm64` child manifest/config digests, and local image ID. Prove exactly six local containers named `ez-mac-runner-b-1..6` use that receipt-bound local ID and execute one captured six-job workflow using direct `docker top` `Runner.Worker` evidence. Snapshot Docker pull events/call counters, unload/load or kickstart the receipt-valid launchd service, and prove restart performs zero pulls while all six slots reconverge. Finally run the repository's 16-runner health/capacity consumer: Task 7's ten Linux evidence remains valid and the six Mac slots are green.

- [ ] **Step 5: Record recovery and land evidence**

Failure injection must prove each nonterminal phase either restores the byte-exact prior binary/config/plist/receipt set and leaves launchd inactive, or recognizes the exact committed poststate without a second pull. Missing snapshots, mismatched records, replayed admissions, or ambiguous artifacts remain inactive `RecoveryRequired`; recovery never starts, pulls, activates, or begins a new transaction. Add the exact source SHA, receipt digest, six-slot proof, restart pull count, and recovery result to the tracked evidence directory and the existing containment Bead, then commit and push that evidence with a `codex/gpt-5.6-sol:` subject. Do not close the Mac deployment criterion until all six local runner slots and the combined 16-runner contract are proven.
