...
installed helper/receipt path, mode, or SHA-256 mismatch     => exit 78
```
...
When invoked from the installed stable path, validate the versioned install receipt rather than comparing with a mutable checkout. The atomically written receipt records the deployed Git SHA and, for `assert-crash-containment.sh`, `ezgha-start-gate.sh`, and the root-owned `proof-allocator.sh`, the source-relative path, installed absolute path, mode, and SHA-256. A missing/malformed receipt or any installed path, mode, or digest mismatch is a policy violation. Changing checkout files after installation does not invalidate an otherwise intact deployment; rollback restores the receipt and helpers together.
...
- Produces: canonical `ezgha topology --json`, canonical read-only `ezgha containment-check --install-static|--install-dynamic|--admission --json` verdicts, an opaque short-lived in-memory `AdmissionPermit`, primitive `install-crash-containment.sh --preflight|--check|--install|--rollback RECEIPT`, and a generated, endpoint-correct main-process startup gate. Static install preflight proves persisted endpoint, hardware, privilege, fresh/upgrade state, current use where applicable, and rollback storage without requiring a reachable VM daemon. Only a recognized persisted Colima/Lima endpoint on a supported platform may then start that exact VM; Docker Desktop returns `NoRecovery`. Dynamic preflight proves the reachable daemon topology/driver and `oomctl` before image work. Neither verdict is an admission capability. `AdmissionPermit` requires the effective installed profile and receipt and is never serialized through the CLI. Task 4 does not edit `install.sh` or retire legacy watcher/exemption state; Task 5 is the sole transaction orchestrator and extends the primitive receipt serially.
...
before any mutation prove persisted endpoint, >=62GiB/32CPU, privilege, fresh/upgrade state, applicable current-use margins, and rollback storage
...
Assert that VM Docker skips the system host profile; contradictory endpoint/daemon metadata, cgroupfs, inaccessible `oomctl`, less than 62 GiB RAM, current use above an activation margin, a control drop-in, or missing privilege causes a nonzero exit before any file write/reload/restart and without printing success. The upgrade branch applies when any live config, generated service, or managed container exists; it requires all three runtime cgroups and their exact margins, so a missing upgrade cgroup fails. The fresh branch requires no live config, no generated service, no managed containers, and no locally registered runner with the proposed prefix; it permits the not-yet-installed cgroups to be absent, reads any that already exist, and proves the static hardware, privilege, rollback-storage, and proposed-unit checks. Its dynamic topology/driver/`oomctl` checks run only after the daemon is reachable, including after the sole allowed exact Colima/Lima start. Mixed state fails instead of being guessed. Include fresh-host success, mixed-state rejection, and 60-GiB/61.99-GiB rejection fixtures. The Task 4 receipt records both `is-active` and `is-enabled`; rollback fixtures cover disabled/inactive, enabled/inactive, and enabled/active prior states.
...
Own `BackendTopology` and `ezgha topology --json` in `src/backend.rs`. The normalized explicit endpoint plus daemon metadata produce `HostDocker`, `VmDocker` (recognized Colima, Lima, or Docker Desktop), or `Indeterminate`; `Platform.daemon_in_vm` is only metadata input. Create `src/containment.rs` as the single Rust owner of the three explicit `ezgha containment-check` modes and `AdmissionPermit`; bare `containment-check --json` is rejected. Static-install, dynamic-install, and admission CLI modes expose typed read-only verdict JSON only. The permit has private fields, records topology, normalized endpoint, and check time, expires quickly, cannot be constructed by callers, and is returned only to internal Rust callers after the exact effective host invariants and installed receipt pass. On Linux, require a fresh `AdmissionPermit` before `prepull_probe_image` and inside `ensure_count_outcome` before stale-container removal, JIT registration, or `docker run`. Thread the permit into every admission-capable helper, keep `start_one` private, and add a compile-time/API fixture that fails if those functions accept an unguarded call. `Commands::Start` therefore cannot bypass the guard, `Commands::Serve` obtains an initial permit before pre-pull and renews inside every reconciliation tick, and direct `ezgha serve` is no less protected than systemd startup. A canonical host endpoint that is temporarily unreachable returns transient failure and never starts Lima; only an explicit recognized Colima/Lima endpoint may receive the separate bounded recovery decision below, after which full classification must succeed. Docker Desktop produces `NoRecovery` when stopped. The shell wrapper calls `--admission --json` for early failure but is defense in depth.

Define the pre-daemon `RecoveryDecision` separately from admission: it is derived only from the persisted normalized endpoint and platform, is not a runner capability, and can authorize only the existing bounded start of the exact configured Colima/Lima backend on a supported platform. Canonical host, Docker Desktop, remote, inherited-context, indeterminate, and unsupported-platform endpoints always produce `NoRecovery`; tests pin the stopped-Docker-Desktop transient failure. After a permitted VM start makes the daemon reachable, Rust must rerun full topology classification and obtain a real `AdmissionPermit` before pre-pull, stale reconciliation, JIT registration, or runner creation. Tests prove the recovery decision cannot reach Docker runner commands or convert a failed HostDocker admission into VM fallback.
...
Make `install.sh` the single deployment transaction and the sole owner of commit/rollback ordering; update `CLAUDE.md` Gate 0 so Linux deployment invokes this entrypoint and removes raw `cargo install --path .` plus direct restart as an accepted substitute. Task 4 exposes idempotent primitives and a receipt but never commits a partial transaction independently. It accepts `EZGHA_PROPOSED_CONFIG` only as a mode-`0600` regular file. After an allowed local `cargo build --locked --release` creates the exact candidate binary, require the static install verdict against that proposed config before any mutation. For a recognized explicit Colima/Lima endpoint on a supported platform only, static success may authorize starting that exact VM; Docker Desktop returns `NoRecovery`, and the dynamic Docker-backed verdict must then pass. Host Docker never recovers a VM. Add an ordered fixture whose Docker stub records build, pull, start, restart, and run: the recoverable Colima/Lima VM start is allowed only between successful static and dynamic checks, while image/Docker runner mutation requires both; every forbidden call fails the test. A failed phase exits without image work or policy/service mutation. Only after both applicable phases succeed may the transaction acquire/build the image on a fresh host; neither verdict is an admission permit. The transaction snapshots the live config; every old policy/helper file; the helper receipt; exact generated `ezgha.service` and `ezgha-alert@.service` units; the live `10-oomd-omit.conf`; the installed legacy watcher service unit, timer unit, and stable script; and the applicable active/enabled states of `ezgha.service`, `ezgha-alert@.service`, `psi-oom-watcher.service`, and `psi-oom-watcher.timer`. It then atomically installs the proposed config; installs stable assertion/gate helpers; verifies each installed path/mode/SHA-256 against source; atomically commits the versioned receipt; invokes the privileged installer including atomic watcher/exemption retirement; and verifies effective containment and `oomctl` roots while the old runner service remains running. It must then require a successful admission verdict before invoking endpoint-preserving `ezgha install-service`; that Rust command independently obtains and consumes the actual in-memory `AdmissionPermit` before regenerating/restarting the unit. Post-start verification checks both unit properties and the new MainPID's `/proc/<pid>/oom_score_adj`. Any post-mutation failure first restores every recorded file byte-for-byte, including helpers, receipt, and both generated units, then reloads the relevant system and user managers, then restores each unit's prior enabled/disabled state and starts only units that were formerly active, and finally verifies the rollback. Fixtures cover helper tampering, receipt tampering/missing/wrong path/wrong mode, checkout changes after install, companion alert-unit rollback, stopped-Docker-Desktop `NoRecovery`, and rollback restoration. An ordered fixture proves the restored old `ExecStart`, not the failed generated command cached before reload, is the process launched. A failed rollback is reported distinctly and never as installation success. Do not add an unconditional Lima dependency or a second service-start path.
...
Expected: after a local candidate build, `install.sh` reruns the immutable preflight against the proposed config before any image acquisition or Docker/live mutation, then acquires the image, snapshots all transaction state, atomically installs the proposed config, installs the exact helpers and versioned receipt, applies and verifies the persistent hierarchy while the old runner service remains running, and regenerates the user unit with the actual installed binary and normalized endpoint. System `actions.slice` and user workload properties match the specification; no systemd control drop-in or live `10-oomd-omit.conf` exists; effective `ezgha.service` OOM preference/score and `/proc/$MainPID/oom_score_adj` are normal; `user@UID.service` retains the same live MainPID and reports `ManagedOOMMemoryPressure=auto`; installed helper path/mode/digests match the committed receipt. If any config, containment, generated-unit, restart, or post-start verification fails, the transaction restores the prior config, system/user policy, helpers and receipt, exact prior user unit, retired watcher/exemption files, and every prior active/enabled state, verifies that rollback, and does not claim success.
