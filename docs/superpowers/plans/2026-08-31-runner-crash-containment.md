...
- Produces: canonical topology and three read-only containment verdicts; synchronous `RuntimeAdmission`; background non-capability `RuntimeAttestation`; `AdmissionIssuer` with one-use `AdmissionMutation`; narrow `ReliefCleanup`; private `TransactionAdmission`; deployment classifier; canonical runner/control locks; durable relief/recovery state; receipt-bound descriptor handoff; primitive containment install/rollback; and the endpoint-correct private systemd startup gate. Static/dynamic verdicts remain non-capabilities; only one exact explicit Colima/Lima start may recover. Runtime proof binds final artifacts/count, background refresh stays outside foreground budgets, every automatic Docker/GitHub/ledger/quarantine/workspace/reaper/canary side effect remints, transaction admission can only install/reload staged units, and the handoff gate transfers the continuously locked FD only to the proven systemd peer. No capability is CLI-serialized. Task 4 does not edit `install.sh` or retire legacy state; Task 5 owns transaction/quiescence/handoff/recovery.
...
Update `src/service.rs::install_systemd` so direct Linux installation obtains fresh `RuntimeAdmission` before writing, enabling, or starting a unit; direct `ezgha install-service` cannot install a bypass when containment is missing. Transactional deferred installation instead requires private `TransactionAdmission` and cannot enable or start. The ordinary path records both unit bytes plus active/enabled intent, writes/reloads/enables, and restarts the new private-mode unit when previously active. On failure, restore both units and reload. Restart prior state only when its receipt/wrapper proves the supported private startup contract; otherwise disable the restored pre-baton main unit, record prior intent in `RecoveryRequired`, and leave it inactive. Add generated-unit and fixture tests for no-admission zero writes, active regeneration, receipt-proven prior restart, and pre-baton disabled/inactive recovery. Backend tests prove zero VM commands for Host Docker/Desktop/remote and at most one exact stopped Colima/Lima start.

Promote canonical `serve.lock` to the installation fence. `start`, `serve`, `stop`, and the transaction use active `state_dir`; static verdict JSON returns it. Release builds reject `EZGHA_SKIP_LOCK`; upgrades reject state/target/scope/prefix drift. Task 5 releases `emergency.lock` before stop/wait, reacquires it to publish `Preparing`, and retains an inheritable exclusive open-file description through mutation. Rust validates bounds/inode and second-description contention. Legacy drain is bounded; deferred install requires the FD and cannot start. Final candidate or compatible rollback uses a mode-`0600` reserved Unix socket, exact systemd peer proof, and `SCM_RIGHTS` to transfer that same locked description before `HandoffLocked`/ACK; no installer copy closes earlier. Pre-baton rollback restores bytes but remains disabled/inactive. Fixtures pin FD continuity, peer/socket/artifact proof, ordinary restart, all death/emergency boundaries, and selected-state start.
...
Define the pre-daemon `RecoveryDecision` separately from admission as the sole backend-start authority. Derived only from the persisted normalized endpoint and platform, it may execute at most one bounded start of the exact configured stopped Colima/Lima backend on a supported platform; it cannot restart a running VM and is never retried by a generic recovery loop. Canonical host, Docker Desktop, remote, inherited-context, indeterminate, and unsupported-platform endpoints always produce `NoRecovery`; tests pin zero VM commands for them and at most one exact start for stopped Colima/Lima. After a permitted VM start makes the daemon reachable, Rust reruns full topology classification and obtains `RuntimeAdmission` before pre-pull or any `AdmissionMutation`.
...
Ordinary runtime admission must fail while final units are absent. Deferred install validates inherited FD/staged receipt, consumes `TransactionAdmission`, installs/verifies both units, reloads, and cannot start or reach runners. After final admission, invoke private Rust `handoff-service --transaction-lock-fd <n>`. It revalidates the inherited fence, creates a fresh mode-`0600` canonical `state_dir/handoff.sock` without following/replacing an existing path, and publishes `HandoffReserved` containing socket identity, protocol, nonce, transaction/installer/helper identities, deadline, mode, unit, and exact artifact digests. While retaining the FD and without `emergency.lock`, it submits exactly one no-block/fail systemd job through a five-second bounded executor. The generated wrapper invokes hidden `serve --systemd-private`.

Private mode uses five-second read-only manager queries to prove exact lowercase invocation ID, MainPID, normalized unified-v2 cgroup path ending in `ezgha.service`, FragmentPath, executable/config, receipt artifacts, and live installer/helper identities. It CASes matching `Reserved -> Claimed`, connects to the reserved socket, and sends nonce/PID-start/invocation evidence. The helper requires matching `SO_PEERCRED` plus `/proc`, state, receipt, cgroup, and digest proof, then transfers the already-locked FD with `SCM_RIGHTS`. The claimant validates bounds and `fstat` identity, requires idempotent nonblocking flock success on the received shared open-file description and `EWOULDBLOCK` on an independent canonical description, CASes `Claimed -> HandoffLocked`, and sends `FD_ACCEPTED`. Only after verified acknowledgment/state may the installer close its copy, so the fence has no unowned interval. The helper closes the listener, identity-checks/unlinks only the reserved socket path, fsyncs its parent, verifies absence, and sends nonce-bound `CLEANUP_COMPLETE`; the claimant independently proves absence before `ServiceOwned`. On EOF/timeout, the claimant may perform the same cleanup only after proving the reserved helper PID/start identity dead and therefore its listener closed. A live/unproved helper or replacement mismatch is `RecoveryRequired`. With no transaction/nonterminal handoff/latch, ordinary private systemd boot acquires the lock normally and uses the shared terminal-owner resolver; direct `start`/`serve` use the same resolver after locking. Other state fails `78` before backend work.
