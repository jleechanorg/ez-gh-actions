...
The task list below is the detailed reference, not permission for another design cycle. Execute it through three test-first milestones:

| Milestone | Existing tasks | Required executable checkpoint | Stop condition |
|---|---|---|---|
| 1. Finite boundary | Tasks 1 and 3 | Policy artifact tests and the hermetic assertion move RED to GREEN. | At 30 minutes without RED or GREEN, stop adjacent work and report the exact command/blocker. |
| 2. Fail-closed admission | Task 2 | Focused Rust admission/ancestry tests move RED to GREEN, then the nearest Rust suite passes. | Do not begin activation while the focused suite is red. |
| 3. Canonical activation | Tasks 4 and 5 | Activation/installer fixtures move RED to GREEN and prove ten contained slots or an inactive service. | At two full gate cycles or three total hours, stop and report remaining failing criteria. |

Task 6 and Task 7's expensive 24-hour outcome evidence run once and last, after the three milestones are green. Review occurs once on the integrated blocker surfaces. A changed SHA invalidates evidence only when the changed files intersect it. Style, wording, optional hardening, new artifacts, and unrelated findings are follow-ups and cannot block these milestones.
...
- Exit 0 means `MemTotal >= 65,011,712 KiB`, at least 32 online logical CPUs, and every fixed policy, Docker mode, removal condition, container count, and ancestry assertion passed.
...
Stub `docker`, `systemctl`, and `systemd-run` through a fixture `PATH`. Cover exact 65,011,712-KiB/32-CPU pass, `MemTotal` one KiB below the floor, 31 online CPUs, missing/malformed memory or CPU-online data, one wrong slice value, `max`, wrong Docker driver, each of `cpu`, `io`, `memory`, and `pids` missing independently from root `cgroup.controllers`, active exemption, present unlimited override, nine containers, eleven containers, and one wrong PID ancestry. Every host-floor or missing-controller case emits a stable `FAIL:` and records no mutation.
...
MemTotal >= 65,011,712 KiB from /proc/meminfo
Linux CPU range-list in /sys/devices/system/cpu/online resolves to at least 32 online logical CPUs
...
- Production activation runs only from the fixed `$HOME/.local/libexec/ezgha/release1` bundle staged by Task 5. The exact root manifest keys are `apply-host-containment-release1.sh` and `assert-host-containment-release1.sh`, both mode `0755`; tracked policy paths are otherwise preserved alongside rendered `systemd/ezgha.service`, the explicit Linux auxiliary closure, `Dockerfile.runner`, and `docker/tar-workspace-wrapper.sh`. Activation derives `BUNDLE_ROOT` from its resolved executable directory, calls only its sibling root assertion, and rejects caller/checkout assertion paths. `build_release1_runner_image()` takes no arguments, derives its Dockerfile/context from that canonical bundle, and is the sole shell bootstrap-build interface.
...
MemTotal below/missing/malformed 65,011,712-KiB floor
online CPU range below/missing/malformed 32-CPU floor
...
Run `uname -s` first and require exact `Linux`, then reject effective UID 0. Resolve the fixed deploy UID from real/effective UID equality and its canonical home through `getent passwd`; require `HOME` equality, the user bus at `/run/user/<uid>/bus`, a successful bounded read-only `systemctl --user show-environment`, and the exact canonical release bundle/manifest with root `apply-host-containment-release1.sh` and `assert-host-containment-release1.sh` keys at mode `0755`, required policy files, rendered `systemd/ezgha.service`, the explicit auxiliary closure, `Dockerfile.runner`, and executable `docker/tar-workspace-wrapper.sh`. Reject a missing/wrong-mode/SHA-mismatched key, absolute or `..` key, path resolving outside `BUNDLE_ROOT`, and old `scripts/host/` production key before intent, service stop, or policy mutation. Require `MemTotal >= 65,011,712 KiB` from `/proc/meminfo` and parse `/sys/devices/system/cpu/online` Linux range-list syntax to require at least 32 online logical CPUs; missing, malformed, or lower values emit stable `FAIL: host-size ...` diagnostics and exit before intent, lock, service stop, `sudo`, Docker, or policy mutation. Also require no secondary `ezgha serve` or canary service, at least 8 GiB `MemAvailable`, swap use at most 50%, memory PSI `full avg10 < 1.00`, at least 1 GiB free on the target filesystem, cgroup v2 root `cgroup.controllers` containing `cpu`, `io`, `memory`, and `pids`, unset Docker selector variables, Docker 29-compatible systemd/cgroup-v2 at `unix:///var/run/docker.sock`, exact count 10, 2500-MiB runner memory, and `actions.slice` parent. Migration requires readable `agents.slice/memory.current < 18G`, `automation.slice/memory.current < 4G`, and exactly ten total managed containers with the expected names/ancestry plus `actions.slice/memory.current < 26G`. Bootstrap requires zero managed containers and permits the three workload slice cgroups to be absent/inactive; it records absent user-slice use as zero only in this branch and later starts and verifies them before build/admission. Construct every later elevated argv, pre-authorize each with `sudo -n -l -- <exact argv>`, and on any denial emit stable `SUDOERS_PREREQUISITE_MISSING` and fail before intent/service mutation without prompting.
...
Record Docker version, `CgroupDriver=systemd`, `CgroupVersion=2`, exact `unix:///var/run/docker.sock` resolution, `MemTotal >= 65,011,712 KiB`, at least 32 CPUs from `/sys/devices/system/cpu/online`, current ten container IDs, PID ancestry, exact current system/user slice values, `actions.slice/memory.current`, free memory/swap/disk, and PSI. Abort activation on any fixed threshold or profile mismatch.
