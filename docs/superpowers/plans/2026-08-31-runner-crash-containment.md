...
Checkpoint 4D owns only `.github/workflows/runner-image.yml`. Validate its hosted-runner-only selector, exact three-path trigger set including the workflow itself, package permissions, digest/provenance artifact schema, public GHCR target, and absence of self-hosted labels; run `git diff --check`, then add, commit `codex/gpt-5.6-sol: publish immutable runner image`, and push. That workflow-path push triggers the hosted publisher and makes its digest artifact available without using the production runner fleet.

Checkpoint 4E owns startup/profile/image-lock integration: `scripts/host/ezgha-start-gate.sh`, `src/service.rs`, `src/containment.rs`, `src/main.rs`, `src/alert.rs`, `config/config.toml.linux.example`, and `config/runner-image.lock`. Download the successful 4D run's machine-readable provenance artifact with `gh run download`, generate the lock structurally, require its publisher source commit to equal the prior 4D commit, and separately require its workflow/Dockerfile/context digests to match the current tree before adding it; the later lock/profile commit does not pretend to equal the publisher commit. Run shell syntax, generated-unit/service/alert tests, config/JIT exact-four-label tests, publisher/lock provenance fixtures, `cargo test`, `bash tests/assert_crash-containment_test.sh`, and `git diff --check`; then run `git add scripts/host/ezgha-start-gate.sh src/service.rs src/containment.rs src/main.rs src/alert.rs config/config.toml.linux.example config/runner-image.lock`, commit `codex/gpt-5.6-sol: wire contained service startup`, and push.
