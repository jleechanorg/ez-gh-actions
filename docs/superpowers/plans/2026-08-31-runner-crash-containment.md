...
Implement the fixed privileged broker at `/usr/local/libexec/ezgha/ezgha-privileged` as a root-owned non-symlink mode-`0755` installation of the tracked Rust binary. It reads only root-owned non-symlink metadata that binds its own digest, deploy UID/GID, passwd-derived containment root, fixed candidate inbox, installed binary, and administrator-authorized candidate digest. It accepts exactly `transaction`, `recover-transaction --confirm-transaction <nonce>`, and `clear-relief --confirm-request <id>` and rejects every path, shell command, environment override, unknown flag, and extra argument. For `transaction`, it opens the fixed manifest and candidate with no symlink traversal and requires the retained candidate descriptor's digest to equal both the manifest and independent root-owned authorization. A self-consistent substituted manifest/candidate must produce zero effects. The broker executes the closed transaction state machine itself and installs the candidate only for later deploy-UID service execution; it never root-executes candidate or installed application code. Recovery and clearance also execute inside the broker. Normal transactions never replace the broker or authorization; a new helper protocol or candidate digest requires the tracked interactive bootstrap before quiescence. `install.sh` proves this exact `sudo -n` contract before quiescence and ends with `exec sudo -n -- /usr/local/libexec/ezgha/ezgha-privileged transaction`; it never sudo-executes a mutable checkout path. The broker validates the explicit deploy UID/GID and minimal receipt-bound environment, retains root only for system-manager and system-path effects, and spawns every user-manager operation after `setresgid`/`setresuid` with no arbitrary environment inheritance. Missing privilege, helper, authorization, metadata, candidate/installed-binary identity, post-validation pathname stability, or any mismatch exits with zero effects. Only then may it revalidate typed verdicts, snapshot replaced policies/helpers/generated units plus active/enabled intent, acquire and retain the canonical transaction-lock FD, and become the sole owner of live mutation. No gate accepts an arbitrary command, path, callback, environment, or shell-produced capability. Each final artifact rename/mode/owner change, policy/unit install/removal, manager reload/enable, and allowed service-start spawn is one separately prepared `DeploymentGate` effect. Each rollback/stop/restore/reload is a separate latch-bound `ReliefGate` effect, starts nothing, and leaves durable `RecoveryRequired`/`Mixed` state rather than shell-retrying an ambiguous result. Neither shell surface calls `systemctl set-property`, and compatible prior active state may return only through receipt-proven descriptor handoff; pre-baton rollback remains disabled/inactive `RecoveryRequired`.
...
Make `install.sh` the single public deployment entrypoint and sole shell preflight/build/stage dispatcher; Task 4's Rust broker remains the sole owner of quiescence, locks, durable state, live commit/rollback ordering, and manager effects. Add `scripts/host/bootstrap-ezgha-sudoers.sh` as the sole explicitly interactive administrator operation before a fresh deployment or candidate/helper digest change. It verifies the deploy account and passwd home, requires a clean exact candidate build, atomically installs `/usr/local/libexec/ezgha/ezgha-privileged` as root-owned non-symlink mode `0755`, and writes root-owned metadata containing the helper digest and independently authorized candidate digest. It renders a user-specific `systemd/host/ezgha-containment.sudoers.template` as root-owned mode `0440`, validates it with `visudo -cf`, atomically installs it, and proves the deploy owner can use `sudo -n` for the three exact helper grammars. The target Ubuntu sudo version must pass a temp-root fixture for anchored command-argument regexes matching only `^transaction$`, `^recover-transaction --confirm-transaction <strict-nonce>$`, and `^clear-relief --confirm-request <strict-request-id>$`. The sudoers rule uses `env_reset`, grants no `SETENV`, and authorizes no shell, extra argument, or caller-selected path; the helper independently repeats the strict grammar checks. Candidate bytes changing after authorization fail before quiescence. Normal transactions cannot update the helper or authorization.
...
After `install.sh` `exec`s the fixed broker, Task 4's broker-owned Rust transaction internally invokes deferred unit installation, final admission, and `handoff-service`; it alone owns the transaction FD, typed state, effect gates, wait/deadline handling, and rollback. Task 5 ends at exact fixed-helper dispatch. Its integration fixtures prove private staging, the three exact helper argv grammars, `BootstrapPrivileged` path/effect/rollback limits, sudoers validation, independent root-owned candidate authorization, helper/candidate/receipt descriptor identity, exit-status propagation, toolchain-independent clearance, and no surviving shell or shell-side live effect. Task 4 replaces/deletes candidate and recovery pathnames after descriptor verification and supplies a self-consistent substituted manifest/candidate; every case proves zero lock/state/gate/backend/manager/final-path effect unless the retained candidate descriptor also equals the independent root authorization. FD preservation, `READY`, typed handoff outcomes, rollback, peer proof, `SCM_RIGHTS`, close-on-exec, socket cleanup, terminal-owner CAS, death windows, and deadline boundaries are Task 4 Rust tests committed with its Rust files.
...
The Task 5 inventory contains 55 paths after adding the two bootstrap artifacts; the earlier review's 57-file description was a count error. Do not invent files to match it. Task 4 must be fully green first, and no live deployment or pressure proof runs between these checkpoints.
...
bash -n install.sh scripts/host/install-crash-containment.sh scripts/host/bootstrap-ezgha-sudoers.sh \
  ...
  docs/test-cleanup-stuck-runs.sh docs/test-queue-backlog-drain.sh
...
git add install.sh scripts/host/install-crash-containment.sh scripts/host/bootstrap-ezgha-sudoers.sh \
  ...
  docs/test-cleanup-stuck-runs.sh docs/test-queue-backlog-drain.sh scripts/host/assert-qemu-cpu-ceiling.sh \
...
Checkpoint 5C owns doctors, Gate 8, and topology consumers:
...
git diff --check
git add doctor.sh doctor-runner docs/EXIT-CRITERIA.md docs/verify-exit-criteria.sh \
  tests/doctor_runner_host_pressure_test.sh tests/verify_exit_gate8_test.sh \
...
After that read-only preflight passes and before quiescence, run `scripts/host/bootstrap-ezgha-sudoers.sh` from the clean reviewed tree. It builds/verifies the exact candidate, asks for administrator privilege only in this step, installs the root-owned broker/sudoers/authorization atomically, and records the authorized candidate digest. Rerun the read-only preflight and require the staged candidate digest to equal that root-owned authorization. A source, lockfile, feature, or candidate-byte change after authorization returns to this step; it never silently reauthorizes during deployment.
