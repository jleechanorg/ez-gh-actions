...
Promote canonical `serve.lock` to the installation fence. `start`, `serve`, `stop`, and the transaction use active `state_dir`; static verdict JSON returns it. Release builds reject `EZGHA_SKIP_LOCK`; upgrades reject state/target/scope/prefix drift. Task 5 releases `emergency.lock` before stop/wait, reacquires it to publish `Preparing`, and retains an inheritable exclusive open-file description through mutation. Rust validates bounds/inode and second-description contention. Legacy drain is bounded; deferred install requires the FD and cannot start. Final candidate or compatible rollback uses a mode-`0600` reserved Unix socket, exact systemd peer proof, `SCM_RIGHTS`, and receiver close-on-exec to transfer that same locked description before `HandoffLocked`/`FD_ACCEPTED`; no installer copy closes earlier, cleanup precedes ownership, and fixed deadlines fit `TimeoutStartSec`. Pre-baton rollback restores bytes but remains disabled/inactive. Fixtures pin FD continuity/noninheritance, peer/socket/artifact proof, boot-stamped ordinary restart, all death/emergency boundaries, and selected-state start.
...
There is exactly one automatic mutating queue/reaper executor: `serve`'s private `ReconciliationCoordinator`. `ReaperPlanner` and `ezgha reaper-plan` are read-only; `ReaperExecutor` is private to the coordinator and receives the issuer/gate. Move queued-run and zombie cleanup into that loop, execute at most one candidate chain per iteration, and gate workflow cancel, optional force-cancel, runner-registration delete, ledger release, and later refill separately. A denied, cancelled, expired, failed, or indeterminate primitive ends the chain and iteration; a later iteration must rediscover state read-only and never generically replay an unknown remote outcome. Make `scripts/cleanup-stuck-runs.sh` permanently read-only by removing `--apply`; delete the queue-reaper systemd service/timer and launchd stopgap script/plist, and transactionally stop/disable/remove installed copies. Installation fails verification if any retired queue-reaper job remains active or enabled. Split the current reaper API so planner/docker modules cannot call GitHub mutation functions except through the private coordinator.
...
Latch acknowledgment changes transaction state to `ReliefLatched`; no runner-capable operation begins afterward. Emergency waits at most 35 seconds for acknowledgment, may stop `ezgha.service`, and leaves every durable request on timeout. Recovery has two tracked forms and never starts backend, service, or runner. Non-emergency `./install.sh --recover-transaction --confirm-transaction <nonce>` exclusively acquires `serve.lock` without holding `emergency.lock`, rejects any unresolved request, requires service inactive, proves no live transaction/service owner, and accepts only matching `Preparing`, `Fenced`, `HandoffReserved`, `HandoffClaimed`, `HandoffLocked`, `SubmissionUnknown`, or `RecoveryRequired`. It completes/verifies saved rollback or proves no mutation, removes stale socket state, fsyncs `RecoveredInactive`, and leaves pre-baton state disabled/inactive while permitting a later clean installer retry.

Relief `./install.sh --recover-transaction --confirm-request <id>` performs the same fence, inactivity, dead-owner, and rollback/no-mutation proof, validates that exact request ID/digest, then tombstones only that request under `emergency.lock` and fsyncs the collection. Other requests do not block targeted clearance; they remain durable and keep every startup/admission path blocked. Clearing the final request archives `RecoveredInactive` only when a valid transaction snapshot exists. When no transaction ever existed, it fsyncs `no_transaction_mutation`, creates no recovery archive, and a later ordinary start independently performs full admission. Busy fence, live owner, active service, wrong identity, malformed/unknown state, or inconsistent snapshot remains fail-closed. Fixtures cover every stranded state, wrong nonce, first pre-baton migration failure/retry, repeated requests cleared in either order, remaining-request blocking, emergency before `Preparing` followed by final clear and ordinary admission/start, malformed/mismatched/no-nonce requests, ownership-limited removal, and zero GitHub or backend start/refill/create calls.
...
bash tests/assert_crash_containment_test.sh
git diff --check
git add scripts/host/install-crash-containment.sh scripts/host/ezgha-start-gate.sh src/backend.rs src/containment.rs src/config.rs src/service.rs src/main.rs src/docker_backend.rs src/github.rs src/reaper.rs config/config.toml.linux.example tests/install_crash_containment_test.sh
...
- Produces: one atomic installation path, retirement of the conflicting watcher/exemption stack and every independently scheduled mutating queue reaper, read-only topology/reaper diagnostics, and a CI-enforced hard harness gate.
...
Retire the independent queue-reaper stack in the same transaction. Stop/disable and remove installed `ezgha-queue-reaper.service`/`.timer`, the launchd stopgap job, and stable stopgap executable; delete their tracked source artifacts; remove installer auxiliary-unit loops that recreate them; and remove `cleanup-stuck-runs.sh --apply` so the remaining script and `ezgha reaper-plan` are read-only. Verify no matching systemd unit or launchd job is active/enabled and no active source invokes the removed apply path. Forced-failure fixtures restore prior external reaper bytes/state exactly; successful installation leaves only the supervisor-owned gated reaper. No intermediate commit may remove a source while an active installer/test still references it.
...
Only after quiescence, `Preparing`, fence acquisition, and nonce-matched `Fenced` may the transaction acquire/build the image; install candidate binary, mode-`0600` config, helpers and policy; stage/verify both units; and atomically commit the receipt containing final config path/mode/digest/count plus every artifact. Config and receipt artifacts are one rollback unit. The shell performs reads and staging only; Rust routes every live artifact/policy/unit commit and manager reload/enable/start spawn separately through `DeploymentGate`, with `TransactionAdmission` additionally required for deferred unit commits. It also checks durable relief before/after each bounded operation, before deferred install, at least once per second during drain, and through handoff. Latch acknowledgment atomically records `ReliefLatched`, blocks every later forward gate, rolls back under `serve.lock` through `ReliefGate`, and leaves the supervisor inactive. Stale/malformed/mismatched transaction or latch state remains fail-closed until exact verified recovery/clearance.

Ordinary runtime admission must fail while final units are absent. Deferred install validates inherited FD/staged receipt, consumes `TransactionAdmission`, installs/verifies both units, reloads, and cannot start or reach runners. After final admission, invoke private Rust `handoff-service --transaction-lock-fd <n>`. It revalidates the inherited fence, sets and verifies `FD_CLOEXEC` on its own descriptor copy before spawning any child, and leaves the installer's separate descriptor flag untouched; the shared lock and later `SCM_RIGHTS` transfer remain valid while `systemctl` cannot inherit the fence. It requires canonical `state_dir` to be a non-symlink deploy-UID-owned mode-`0700` directory, creates a fresh mode-`0600` `state_dir/handoff.sock` without following/replacing an existing path, and publishes `HandoffReserved` with socket identity, protocol, nonce, transaction/installer/helper identities, `connect_deadline_boottime = reserve + 25s`, mode, unit, and exact artifact digests. The five-second no-block/fail systemd submission, 15-second wrapper assertion, and five-second aggregate provenance/claim/connect budget are non-additive and must finish by that deadline. `HandoffClaimed` records `ready_deadline_boottime = claimant entry + 120s` for peer proof, FD transfer, cleanup, admission, ownership, pre-pull, and `READY=1`. Worst case is 145 seconds, leaving five seconds inside `TimeoutStartSec=150`; no retry, rollback, or transition refreshes either deadline. The generated wrapper invokes hidden `serve --systemd-private`.

Private mode uses one five-second aggregate read-only manager-query budget to prove exact lowercase invocation ID, MainPID, normalized unified-v2 cgroup path ending in `ezgha.service`, FragmentPath, executable/config, receipt artifacts, and live installer/helper identities. The helper submits exactly one no-block/fail systemd job through `DeploymentGate::spawn`: it holds `emergency.lock` only through latch/capability recheck, prepared-record fsync, and exact spawn/identity registration, then releases before the five-second wait. Private mode CASes matching `Reserved -> Claimed`, connects to the reserved socket, and sends nonce/PID-start/invocation evidence. The helper requires matching `SO_PEERCRED` plus `/proc`, state, receipt, cgroup, and digest proof, then transfers the already-locked FD with `SCM_RIGHTS`. The claimant uses `recvmsg(MSG_CMSG_CLOEXEC)`, rejects ancillary truncation or extra/missing FDs, verifies `FD_CLOEXEC`, validates bounds/`fstat` identity, requires idempotent nonblocking flock success on the received shared open-file description and `EWOULDBLOCK` on an independent canonical description, CASes `Claimed -> HandoffLocked`, and sends `FD_ACCEPTED`. Only after verified acknowledgment/state may the installer close its copy, so the fence has no unowned interval and no later Docker/GitHub child can inherit it. The helper closes the listener, identity-checks/unlinks only the reserved socket path, fsyncs its parent, verifies absence, and sends nonce-bound `CLEANUP_COMPLETE`; the claimant independently proves absence before `ServiceOwned`. On EOF/timeout, the claimant may perform the same cleanup only after proving the reserved helper PID/start identity dead and therefore its listener closed. A live/unproved helper or replacement mismatch is `RecoveryRequired`.

`ServiceOwned` records boot ID, PID/start, invocation, lock, admission/artifact, socket-cleanup, and typed origin evidence: handoff origin has transaction/ready deadline, ordinary origin has owner-start boot time and no handoff deadline. With no transaction/nonterminal handoff/latch, ordinary private boot, direct `start`, and direct `serve` acquire `serve.lock` and invoke one shared terminal-owner resolver before backend work. Under `emergency.lock` it reads one generation and performs at most one exact CAS: no record proceeds after blocker recheck; a complete receipt-matching different-boot record archives as `PriorBoot`; a complete same-boot record with absent/reused PID-start archives as `SameBootDead`. Archival additionally requires the manager unit inactive, except for the exact private systemd caller proving self MainPID/invocation/cgroup. A live valid owner, contradictory lock evidence, foreign active MainPID, malformed/missing boot ID or origin/deadline, artifact/lock/invocation mismatch, residual socket, or failed CAS/fsync fails `78` without archive/start; only an independently valid transaction snapshot may become `RecoveryRequired`. Archival fsyncs state/parent and rechecks blockers before admission; one CAS conflict restarts once, a second exits `75`. Archived state is audit-only, and every successful ordinary caller publishes a new boot-stamped ordinary `ServiceOwned` before pre-pull. Fixtures cover prior boot with PID reuse, same-boot death, live/foreign owners, malformed evidence, residual socket, snapshot recovery, and direct/systemd races.

On ambiguous submission, matching unclaimed reservation becomes `SubmissionUnknown`; a fully matching claimed/connected peer may finish the bounded transfer, while any latch, mismatch, or failed peer proof becomes `RecoveryRequired`. No FD is sent after invalid proof or latch. Before ACK failure retains the installer fence until state is unclaimable, then releases it, performs bounded query/stop/inactive verification without either lock, and reacquires before rollback. After transfer but before ownership, failure or relief stops the service, verifies its received FD closed, then reacquires. Matching socket cleanup occurs only after listener closure and receiver-FD closure proof. Inability to stop, prove closure, clean the matching socket, or reacquire remains fail-closed. Fixtures cover accepted-before-timeout, accepted-without-claim, no acceptance, late process, peer mismatch, and emergency at each socket/transfer boundary.

The claimant holding the transferred FD rechecks reservation/latch/deadlines and runs full admission. After cleanup completion plus its own absence proof, or the dead-helper fallback proof, it CASes `HandoffLocked -> ServiceOwned` before pre-pull or runner mutation. Terminal evidence records boot, process, invocation, lock, admission, artifacts, cleanup, and fixed ready deadline; publication therefore cannot leave the reserved path behind, and a live match survives installer death. Pre-pull remains bounded by the same ready deadline, and `READY=1` requires a final latch/deadline check. Expiry or any other post-ownership/pre-READY failure first CASes that exact ownership to `RecoveryRequired`, then exits without runner mutation; it never extends a deadline. Installer READY/bookkeeping waits hold neither lock. Before transfer, installer death makes the claimant refuse; after transfer, a fully proven claimant may complete ownership, otherwise it records recovery, closes the FD, and exits without mutation.

Queue, peer, transfer, ownership, or READY failure uses stop, received-FD closure proof, matching socket cleanup, fence reacquisition, and rollback. Compatible prior artifacts restart only when their receipt/wrapper advertise the same descriptor-handoff protocol; rollback uses the identical socket transfer. Pre-baton rollback restores exact bytes and records prior enabled intent but leaves the service disabled/inactive `RecoveryRequired`; only verified non-emergency recovery may finalize `RecoveredInactive` for a clean installer retry. Fixtures cover `SCM_RIGHTS` lock continuity after sender close; helper-side `FD_CLOEXEC` with a `systemctl` stub that execs a long-lived child; claimant `MSG_CMSG_CLOEXEC` plus a later spawned-child lock-leak rejection; wrong inode/socket/nonce/UID/PID-start/cgroup/digest; exact `5/15/5/120` boundaries and one-millisecond overruns; READY timeout with no socket/received FD; successful handoff followed by a second transaction bind; helper death before cleanup; claimant death immediately after `ServiceOwned`; direct-command races at every state; claimant/helper/installer death before/after transfer and ownership; emergency/socket cleanup at every boundary; supported rollback transfer; pre-baton recovery; normal restart; and no lock during READY, stop, or reacquisition polling.
...
git diff --check
git add install.sh scripts/host/install-crash-containment.sh scripts/cleanup-stuck-runs.sh doctor-runner docs/verify-exit-criteria.sh docs/host-ops-sudo-block-0725.md docs/test-cleanup-stuck-runs.sh .github/workflows/ci.yml tests/install_watchdog_gate_test.sh tests/install_uninstall_aux_units_test.sh tests/host_control_artifacts_test.sh tests/host_ops_0725_test.sh tests/doctor_runner_host_pressure_test.sh tests/verify_exit_gate8_test.sh tests/crash_containment_suite.sh
...
git add CLAUDE.md AGENTS.md README.md roadmap/up-changelog-20260831-crash-containment.md \
  ...
  src/backend.rs src/containment.rs src/config.rs src/docker_backend.rs src/github.rs src/reaper.rs src/service.rs src/main.rs \
