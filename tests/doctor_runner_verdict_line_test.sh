...
#       cycling slots (recovered mid-window) fold into executing, matching
#       section 9's existing "cycling folds into live" semantics.
#
...
run_case() {
  ...
  local out total configured executing idle_ok idle_starved down
  out=$(compute_verdict_summary "$l_exec" "$l_idle" "$l_down" "$l_cyc" \
                                 "$r_exec" "$r_idle" "$r_down" "$r_cyc" "$starved")
  read -r total configured executing idle_ok idle_starved down <<< "$out"
  ...
  if [ "$pass" = "true" ]; then
    echo "  [$label] total=$total configured=$configured executing=$executing idle_ok=$idle_ok idle_starved=$idle_starved down=$down -- PASS"
    return 0
  else
    echo "  [$label] got: total=$total configured=$configured executing=$executing idle_ok=$idle_ok idle_starved=$idle_starved down=$down" \
         "| expected: total=$exp_total configured=$exp_configured executing=$exp_exec idle_ok=$exp_idle_ok idle_starved=$exp_idle_starved down=$exp_down -- FAIL"
    return 1
  fi
}
...
run_case "local14exec-2idle-remote6exec-no-starvation" \
  14 2 0 0  6 0 0 0  0 \
  22 22 20 2 0 0 || OVERALL_PASS=false
...
run_case "local14exec-2idle-remote6exec-starved" \
  14 2 0 0  6 0 0 0  1 \
  22 22 20 0 2 0 || OVERALL_PASS=false

# Case (c): 2 local DOWN + 1 remote DOWN, 1 local CYCLING (recovered
# mid-window, folds into executing per section-9 semantics) -- proves down
# slots are counted and cycling slots are NOT double-counted as down.
run_case "local-2down-1cycling-remote-1down" \
  12 2 2 1  4 1 1 0  0 \
  23 23 17 3 0 3 || OVERALL_PASS=false
