...
**Automated gate:** the pre-push hook (`.githooks/pre-push`) blocks pushes when binary-drift is detected (HEAD SHA ≠ installed-binary SHA after a Rust change); the `binary-sha-drift.yml` CI workflow catches any case the hook misses. Wire the hook via `git config core.hooksPath .githooks` (enables both `commit-msg` provenance-prefix check + `pre-push` binary-drift check). Bypass the local hook with `EZGHA_SKIP_BINARY_DRIFT_CHECK=1 git push` only when you understand the consequence.

**Markdown bodies to `gh api`:** always pipe a structured payload through stdin — never interpolate a Markdown body into a shell-built command. Use `jq --rawfile BODY_FILE . | gh api --input -` (or `jq -Rs <<<"$BODY" | gh api --input -`). The unsafe pattern `gh api ... -f body="$BODY"` evaluates `$BODY` in the shell, so backticks or `$(...)` in the body execute as commands on the build host and their output (or worse, a prompt re-piped through a heredoc) ends up inside the PR / issue / comment body — the PR #56 incident started a legacy Lima VM this way. The pre-push hook also runs `scripts/lint_gh_api_no_shell_interp.sh` on every commit that touches a `.sh`/`.githooks/*`/`install.sh` path; bypass with `EZGHA_SKIP_GH_API_LINT_CHECK=1 git push` only when you understand the consequence.
