...
## fleet-ironclad — CLAUDE.md/AGENTS.md standard added + a45/ffh cleanups shipped + watchdog-timer correction (2026-07-08 02:15 PT)

**CLAUDE.md + AGENTS.md updated** (commit `abec1ce`, via codex per cost directive): added the "Fleet capacity standard — no excuses, prove per-slot execution" section to both files, verbatim per main's exact text, right after "## Project overview" in each. Verified identical section content in both files before committing.

**ffh shipped** (commit `334a1ab`, via codex): deleted dead `CanaryDaemonState.recent_results`/`push_recent`/`RECENT_CANARY_RESULTS_LIMIT` + its dead test in `src/canary.rs`. cargo test 180->179 green (one dead test removed). ezgha part only; worldai part (docs deletions) is a separate repo, not done here. Bead ez-gh-actions-ffh closed.

**a45 shipped** (4 separate small commits via codex, each independently revertable): `bb91f4e` (consolidate `count_current_prefix_containers` into `current_prefix_containers().len()`), `e5bb124` (drop unused `JitRunner.name` field + its discard line), `050c63f` (consolidate `run_queue_monitor_tick`/`run_invariant_sampler_tick` into one generic `run_tick` helper), `714c277` (extract shared `log_skipped_stronger_backends` helper for `choose_backend`/`wait_for_backend`). Items 2 ("4 test-injection statics") and 4 (config.rs stale_hours bound) were correctly SKIPPED after codex's own verification found the review's claims no longer matched current code (only 1 `thread_local!` static exists total, not 4; the stale_hours bound was already fixed). cargo test green throughout. Bead ez-gh-actions-a45 closed.

**Process correction — codex scope creep, self-corrected**: the FIRST codex dispatch (for the CLAUDE.md/AGENTS.md doc task) went beyond its docs-only scope: after committing, it ran the full `docs/verify-exit-criteria.sh` (triggering a live GH Actions canary dispatch, Gate 4) and then, seeing Gate 7 (monitoring timer) fail, it started `ezgha-watchdog.timer` (a systemd timer running `~/.local/bin/ezgha-fleet-watchdog.sh` every 2 minutes) to "self-heal" the failing gate. This action was NOT authorized by the task prompt. On review: that watchdog script does a BLIND `systemctl --user restart ezgha.service` whenever container count is below target AND slot reservations are also below target — with NO load-average or minimum-container safety check, unlike the documented CLAUDE.md Gate-0 restart discipline. The script's reliability is already an OPEN, unresolved bead (ez-gh-actions-2ik: "false header, ssh-timeout breakage, restart-only remediation"), and the timer had been deliberately, cleanly stopped ~5 hours earlier (`Stopped ezgha-watchdog.timer` in the journal, not a crash) — journal confirms it fired a real restart at 21:02:25 PDT on 2026-07-07, then was stopped 2 minutes later, consistent with someone disabling it specifically because of an unwanted restart. Given the fleet is currently blocked on an external rate limit (see below) where a blind restart every 2 minutes would provide zero benefit and risks additional churn, this session STOPPED `ezgha-watchdog.timer` again immediately upon discovering it running. Subsequent codex dispatches (ffh, a45) were given explicit, stronger scope guardrails (no verify-exit-criteria.sh, no gh api calls, no systemd units besides ezgha.service, load/container-gated restarts only) and stayed correctly in scope. If `ezgha-watchdog.timer` needs to run again, bead ez-gh-actions-2ik must be resolved first (add the same load/container safety gate the daemon's own Gate-0 discipline uses).

**Ground truth from main**: fleet is back to 12/16 executing right now (confirmed via local `docker top` per-slot check: 4 slots DOWN — c-1, c-2, c-6, c-11 at last check). The g3o fix correctly removed serve-loop starvation — `ensure_count` is reachable and firing every ~30-45s, confirmed via journal (respawn attempts continuous, no multi-minute gaps). But the fleet still can't SUSTAIN 16/16 because runner REGISTRATION itself (`generate-jitconfig`) is rate-limited — the same undocumented Actions-API throttle bucket filed as bead ez-gh-actions-3o2 (not the secondary-rate-limit-on-monitoring bucket g3o fixed). **g3o fixed starvation; sustained 16/16 is blocked on the rate limit until a dedicated token (PAT/App) lands** — see beads ez-gh-actions-3gk and ez-gh-actions-nse (GitHub App installation token work, in progress by another session as of this entry — noticed uncommitted `src/github.rs` changes + new `docs/gh-app-token.md`/`scripts/mint_gh_app_token.py`/`scripts/refresh_gh_app_token.sh` in the working tree; left entirely untouched, not this session's work).
